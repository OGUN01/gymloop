...
- [x] 7.3 `.github/workflows/ci.yml` — one parallel job per gate (typecheck, lint, knip, jscpd, depcruise, registry-lint, build, test; `escape-hatches` added later, after a verification pass found hard rule #4 was enforced by nothing)
...
- [x] 8.10 PR #1 closed unmerged, branch deleted local and remote, stale tracking ref pruned; no `gateproof` files remain tracked on `main`
...
- [x] 8.13 **Round 2 (PR #2, `chore/gate-proof-2`): all eleven gates re-proven by failure REASON, not exit code**, against the stricter standard the jammed `lint`/`schema-drift` gates exposed. Reasons quoted from CI logs in `docs/gates.md`. This round also proved `escape-hatches` in CI for the first time — it had only ever been proven locally, which by this project's own standard left it unproven. `test` and `pgtap` stayed green throughout, which is the discriminator. Branch deleted, PR closed unmerged, holdout failing test reverted and that suite verified green again.
...
- [x] 9.2 Fresh-context critic sub-agent audited the deliverables against §13's 10 items and §11's 33 gates, with no knowledge of how any of it was built. It earned its place — self-review would not have found most of this:
  - **`registry-lint`, the "constitutional" gate, was bypassable three ways**: `export async function` never matched (`async` sits between `export` and `function`, so every Phase 2+ Route Handler would have been invisible), `export { a, b }` never matched, and registration was a raw substring test — an export named `Role` or `env` passed on unrelated prose. Fixed, with a test per hole.
  - **`scripts/` was never linted at all**, and threw 14 `no-undef` errors the moment it was (no Node globals declared anywhere). Now linted via `lint:root`; the one real magic number it caught is named, not suppressed.
  - **The `eslint-disable`/knip-ignore ban was enforced by nothing** while being stated as constitutional. Now a real CI job (`escape-hatches`) — which caught a false positive in itself on first run, hence matching directive position rather than the bare word.
  - **§9's quality-bar table, the 2026 design direction, the four E2E journeys and the test-layer table existed only in the master prompt** — which `docs/roadmap.md` declares disposable — while three files routed to a "quality-bar table" that was never written. All transcribed into `docs/architecture.md`. Same for the competitor/pricing research, which lived only outside the repo.
  - **Two overclaims I had written**: `decisions.md` said the Supabase token was "set by the user" (it wasn't), and `gates.md` said gate 10 was proven (it wasn't). Both corrected — and the stale `tasks.md` line asserting the token wasn't a blocker is precisely why the missing secret went unnoticed while `main` sat red.
  - `RENEWAL_REMINDER_DAYS` contradicted itself across three files (`+3` vs `-3`, no stated axis) — a bug Phase 5 would have inherited.
  - Folder map listed directories that don't exist as though they did; `scripts/` and `.claude/commands/opsx/` were undocumented orphans.
- [x] 9.3 Archived to `openspec/changes/archive/2026-09-06-phase-0-foundation/`. Which gate proved what is recorded in section 8 above and in `docs/gates.md`. `openspec/specs/` stays empty by design — Phase 0 declared `skip_specs` because it changes no product behavior, and every substantive artifact it produced has a canonical home the master prompt named (`docs/*.md`); a duplicate spec entry would violate the project's own anti-duplication rule. Phase 1 opens `openspec/specs/` with the first real capability
- [x] 9.4 Final commit to `main`, CI verified green
