> **Superseded in places — read `docs/decisions.md` first.** This design was written before implementation and several decisions in it were reversed during the phase. Most importantly, the schema-drift gate described below as `--local`/Docker/"hermetic" was replaced by `--project-id` against Supabase Cloud (ADR-024), and the `SUPABASE_ACCESS_TOKEN` described as needed "for Phase 1's db push" is required by the drift gate itself and is now set. `docs/roadmap.md` points here as the worked template for later phases: it is a template for the *process* (proposal → design → tasks → proof → archive), not a current statement of how the gates work.
