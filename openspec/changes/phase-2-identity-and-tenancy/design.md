...
- **An impersonation session is immutable except for `ended_at`, and its `started_at` is set by the database.** *(Both from the second blind critic, and the first is the same defect as the rule it repairs.)*

  **`started_at` was unconstrained.** `impersonation_sessions_ttl_chk` bounds `expires_at - started_at`, but `started_at` is client-settable with no upper bound, and `app.impersonation_is_live` asks only `ended_at is null and expires_at > now()`. So `started_at = now() + 10 years` with `expires_at = started_at + 2 hours` passes both check constraints **and is live right now, for a decade** — verified against the live database. The TTL bounded the span and left the anchor free, which is precisely the "a future timestamp is not a bound" mistake it was written to fix. A `check (started_at <= now())` cannot express it — `now()` is not immutable and Postgres rejects it in a constraint — so the database sets the value instead of validating it.

  **The end path could retarget the session.** `impersonation_sessions_impersonator_write` pins `ended_at is not null` and nothing else, so `set ended_at = now(), tenant_id = '<another gym>'` satisfies both clauses in one statement. The audit trigger then writes the end row from `new.tenant_id` and `new.reason` — an end row in a gym that was never impersonated, with a chosen reason, while the real target's start row has no matching end. Not an escalation, since an ended session is not live; it is audit forgery, and §6 asserts the opposite twice. `audit_log_impersonation_session_id_fkey` is single-column, so referential integrity does not block the retarget either.

  **One trigger closes both**, `app.impersonation_session_immutable()`, `before insert or update`: on insert it clamps the anchor **forward only** — `new.started_at := least(coalesce(new.started_at, now()), now())`; on update it restores every column except `ended_at` to its old value — **and once `ended_at` is set, nothing may change at all.** *(The implementer asked whether ending should be terminal, having noticed the trigger as specified permitted setting `ended_at` back to null. It should: the end audit row has already been written, so un-ending produces a session that is live again after its own log says it closed, and ending it a second time writes a second end row against one start. "Written once and then only ended" is the sentence; a session that can be reopened is neither.)*

  **Forward only, and the qualifier is load-bearing.** An unconditional `started_at := now()` closes the defect and makes an **expired session unconstructible inside a transaction, anywhere** — `now()` is the transaction timestamp and never advances, so `expires_at > started_at` then forces every expiry into the future. Expiry is half of liveness and §6 leans on it twice: the hook must hand an expired session's actor back an ordinary platform token, and a lapsed session must leave a start with no end. Neither could be tested. Five pgTAP files backdate a `started_at`, including one of Phase 1's, and each would abort before its first assertion. **A future anchor is the defect; a past anchor is ordinary history**, and the clamp does not need to reach the past to close the hole. Found by the visible-suite author while checking the fixture question it was sent — the wider version of it.

  **What a backdated anchor still permits, stated rather than left implicit:** a `super_admin` may write a session whose `started_at` precedes the write. It is detectable — `created_at` defaults to the insert time and is now immutable, so `started_at < created_at` names every backdated row — and it changes nothing about liveness, since `expires_at` is bounded relative to the anchor and the audit row carries its own timestamp.

  **`ended_at` is clamped the same way, and for the same reason.** The trigger took the submitted value verbatim, and the policy pins only `ended_at is not null`, so an impersonator could close a fifty-minute session claiming it ended at its start, or in 2031. Nothing *reads* the value — liveness, the one-open-session index and the audit trigger's `when` clause all read only its nullness — so there is no liveness or privilege consequence, and the audit row's own `created_at` records the true instant beside the claimed one. It is closed anyway, `least(v_ended_at, now())`, because it is the same shape a third time: the anchor was bounded and the terminus left free. A third blind critic found it and correctly declined to block on it.

  **The hook's own lookups need indexes, and §8.6 did not cover them.** Index rule 3 is about *policy* predicates; the hook queries `staff` and `members` by `user_id` alone, and the only indexes on that column are `(tenant_id, user_id) where user_id is not null` — leading with the wrong column, so they cannot serve `where user_id = $1`. That is up to four sequential scans on **every token mint and every refresh**, against a function with a two-second budget whose timeout §2's handler swallows silently: the failure mode is every user holding a claim-less token with nothing in a log to say why. `staff_user_id_idx` and `members_user_id_idx` are added, partial on `user_id is not null` to match the existing shape. The tables are empty today, which is exactly why this is cheap to fix now.

  **The timing keyword is the guarantee, not the trigger's name.** Postgres fires every `before` row trigger ahead of every `after` row trigger; names order triggers only *within* a group. Both audit triggers are `after`, so they see the corrected row unconditionally. This is recorded because the opposite was asserted while briefing the work, and a later reader must not try to "fix" ordering by renaming. The whole rule becomes one sentence a reader can hold — *a session is written once and then only ended* — rather than a list of columns each policy must remember to pin. **And the policy gains `tenant_id = (select app.current_tenant_id())` on both clauses anyway**, because the trigger is a different component and this phase has now been bitten four times by a policy that leaned on one (§8.1). `tenant_id` already leads `impersonation_sessions_tenant_id_started_at_idx`, so index rule 3 is discharged.
...
**There are three audiences, not two, and the sixth name is the third one.** Every other policy in `public` is addressed to *the platform* (gated on `is_platform()` or `= 'super_admin'`) or to *a gym* (gated on the tenant claim plus a role). `impersonation_sessions_impersonator_write` is addressed to **the impersonator** — a session that is neither, because it carries a gym's `tenant_id` and `gym_owner` while belonging to a platform user. Classifying it by what it is *not* gated on puts it on the gym side, and it then looks like a gym-side policy that admits an `UPDATE`, which the spec forbids. **The spec's guarantee still holds and holds structurally:** a gym-side session carries no `impersonation_session_id`, so `current_impersonation_id()` is null, `id = null` is null, and it reaches no row. A meta-test over policy audiences must recognise the third one, or it will keep reporting the impersonator's policy as a gym's.
