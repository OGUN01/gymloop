...
### Requirement: The person who asked is recorded once and cannot be changed
THE SYSTEM SHALL treat `requested_by_staff_id` as fixed from the moment the pause exists, and SHALL refuse an update that alters it.
...
WHEN a session subject to row security inserts a pause carrying `approved_at`, THE SYSTEM SHALL refuse it, whatever role that session holds. A pause is requested and then decided; a row that arrives at the destination was never governed on the way.
...
Once a pause carries a decision, THE SYSTEM SHALL refuse any change to that decision **or to what the decision covers** — the membership it applies to, and the dates it runs between. A second approver cannot overwrite the first, and nobody can move the freeze out from under the approval.
