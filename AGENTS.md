...
6. **`packages/db/types/database.ts` is generated. Never hand-edit it.** Regenerate with `supabase gen types typescript --linked`. CI diffs it against a fresh generation from Supabase Cloud; drift fails the build.
