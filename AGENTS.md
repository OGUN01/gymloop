...
6. **`packages/db/types/database.ts` is generated. Never hand-edit it.** Regenerate with `supabase gen types typescript --local` (after `supabase start`) — not `--linked`, which emits different template output for the same schema (`docs/decisions.md` ADR-024). CI diffs it against a fresh `--local` generation; drift fails the build.
