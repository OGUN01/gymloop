...
select is(
  ...
  array[23::smallint],
  'the spec''s first requirement says the rule is enforced where every writer meets it, not in a caller -- and an INSERT is a writer: membership_pauses grants insert to authenticated under is_front_office(), so a front-desk session could insert a pause already carrying approved_at and approved_by_staff_id in one statement, self-requested and self-approved, and meet no rule at all if only UPDATE were governed. So tgtype 23 is BEFORE + ROW + INSERT + UPDATE, and nothing else. Refusing a freeze still needs no special role and no second person -- the rule governs only the transition into approved, never a rejection -- but that is an asymmetry in which columns and transitions the trigger body inspects, not in which statements reach it; treating the two as the same asymmetry was the earlier mistake pinned here. LIMIT, stated so this is not read as more than it is: the body could still fail to reject a self-approved insert, or grow a clause governing a rejection it should leave alone, and this assertion would not notice either. That behaviour needs a test written from an EARS spec by an author who has not read app.enforce_pause_decision()'
);
