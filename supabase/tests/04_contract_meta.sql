...
select is_empty(
  $$with preview_tables as (
    ...
    select c.relname || '.' || t.tgname
     ...
     where n.nspname = 'public' and not t.tgisinternal
       ...
       and c.relname not in ('staff', 'members', 'platform_users', 'impersonation_sessions', 'attendance', 'membership_pauses', 'follow_ups', 'payments', 'refunds', 'document_counters', 'memberships')
    union all
  ...
  'docs/data-model.md "What a cluster agent must not do", narrowed by design.md 6 and 7 and by the ADR-066 triggers, with NAV-003: every authenticated-writable public table requires its exact enabled ROW BEFORE INSERT/UPDATE/DELETE preview_read_only trigger calling private invoker app.enforce_preview_read_only(). Only that named, correctly shaped trigger is admitted universally; the shared updated_at allowance and eleven existing table exemptions remain. Missing or malformed preview guards fail the same assertion, including on otherwise exempt tables. The preview rule depends on the acting claim and cannot be a row CHECK constraint. Eleven prior exemptions, each for a reason the rule cannot cover. The identity tables (session revocation and the role-change audit row) and impersonation_sessions (the start and end audit rows). attendance: its de-duplication window is per-gym configuration read from organization_settings, so it can be neither an index predicate nor a check constraint, and attendance grants insert to authenticated, so a rule living in a Route Handler is bypassed by a direct supabase-js write; Phase 4 gives the same table a second trigger, closing a member''s open no-show case on the very attendance row that proves they returned (NSH-005) -- a cross-table write onto no_show_cases that a constraint on attendance cannot make, one more reason the table cannot be governed by the rule rather than a second table to add. membership_pauses: the same argument one table over, and found the same way -- pause_approver_role was compared only in a TypeScript Route Handler while membership_pauses_tenant_write is is_front_office() for all commands with no constraint and no trigger, so any front-desk session could approve a freeze through supabase-js, including one they had requested themselves. What the trigger states cannot be a constraint either: the approver must be the acting staff member, must not be the requester, and must hold the role the gym configured in another table. follow_ups: the same attribution argument again (GL030) -- staff_id must be the staff member holding the session and refused for anybody else it names, the same rule as attendance.assisted_by_staff_id and membership_pauses.requested_by_staff_id -- plus a case status that must follow from the contact actually logged rather than from a value the caller supplies, plus two staff who must not be allowed to log against the same case at the same instant, which is an advisory lock taken before the deciding read, not a row-level check. None of that is expressible as a constraint or an index predicate, and follow_ups grants insert to authenticated, so a rule living only in a Route Handler has a supported way round it. payments: attribution is the same argument as attendance and follow_ups one table over, and here it is the only integrity there is -- a manual payment has no provider signature to verify against, so the name of the staff member who took the money is the whole of the audit trail (GL034). The receipt number is a per-gym, per-financial-year atomic allocation from another table, document_counters, which no constraint on payments can perform. The membership extension a paid payment makes is a cross-table write onto memberships, likewise beyond a constraint''s reach. And payments grants insert to authenticated, so a rule living in a Route Handler is bypassed by a direct supabase-js write -- the same reason attendance and membership_pauses are exempt. refunds: the ceiling GL036 enforces is a SUM over sibling rows compared against a value in another table, taken under a row lock -- a check constraint cannot see other rows, and a unique index cannot express a sum. A trigger is the only place each rule meets every writer. document_counters: the receipt counter may only move forward, and it may not be deleted. document_counters_tenant_write is FOR ALL on is_front_office(), so the same predicate that lets a person take money lets them rewrite the book''s numbering; a critic reset it and the next payment collided with a number already issued. A CHECK constraint cannot see the OLD row, so "may only increase" is inexpressible as one, and a rule in a Route Handler is bypassed by a direct supabase-js write. memberships: a membership''s price_paise and currency may not change once periods_granted > 0. The rule needs the OLD row -- "has anything been granted yet, and is the price different from what it was" -- and a CHECK constraint cannot see OLD. memberships_tenant_write is FOR ALL on is_front_office() with no column restriction, so a rule living in a Route Handler is bypassed by a direct supabase-js write. This is not a nicety: cutting the price after a period had been granted handed out a free month for one paisa -- the extension re-scores all the money against whatever the price is now, so a cut retroactively makes past payments buy more, and the next payment of any size releases it, and one front-desk session can do both halves. That argument is what makes these exemptions rather than the state machines this assertion exists to keep out -- an exemption is a trigger that could not have been a constraint, not a trigger someone preferred. This assertion has now moved six times -- twice in Phase 3, once in Phase 4, three times here in Phase 5 -- and every move was a real change in the schema''s shape that it caught, never churn: that is what a tripwire assertion looks like when it is doing its job, and the correct response each time is to name the new trigger and say why the rule cannot cover it, never to loosen the assertion into a count that cannot fail. VERIFIED against the live catalogue, which the earlier version of this message said it was not: both migrations are now applied to Cloud (`supabase migration list --linked` shows 20260910130000 and 20260910210000 present, and a plain pg_trigger read reports document_counters_monotonic [app.enforce_counter_monotonic] and memberships_price_frozen [app.enforce_membership_price_frozen] alongside each table''s touch_updated_at trigger and nothing else). So document_counters'' exemption is no longer written against a trigger merely specified, and memberships'' is no longer resting on a spliced throwaway copy -- both are read from the catalogue as it stands. STILL NOT VERIFIED, and kept distinct from the above rather than blurred into it: the round-seven rename of the memberships trigger. memberships_price_frozen / app.enforce_membership_price_frozen() become memberships_terms_frozen / app.enforce_membership_terms_frozen() -- still one AFTER UPDATE FOR EACH ROW trigger on the same table, so the exemption this assertion grants is unchanged and this assertion cannot tell the two apart; the message is written against the new name because that is what the schema is becoming, while live Cloud still shows the old one. The reason for the rename is the reason the exemption is widening: the rule freezes the plan a period is measured in as well as the price and the currency, and a second rule under it -- periods_granted is written by the granting rule and by nobody else -- likewise needs the OLD row and likewise cannot be a CHECK. Neither the rename nor the widened rule is applied yet'
);
