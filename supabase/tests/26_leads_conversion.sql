...
insert into public.leads(id,tenant_id,branch_id,full_name,phone,email,source,stage,assigned_to_staff_id,trial_at,converted_member_id,converted_at,lost_reason,notes,created_at,updated_at) values
 ...
 ('58000000-0000-4000-8000-000000000609','58000000-0000-4000-8000-000000000001','58000000-0000-4000-8000-000000000011','Ishaan Roy','+915800000609',null,null,'walk_in','trial_done',null,'2026-09-18T10:00:00+05:30',null,null,null,null,'2026-09-01T10:00:00+05:30','2026-09-18T11:00:00+05:30'),
...
select results_eq($$select count(*) from public.members where tenant_id='58000000-0000-4000-8000-000000000001' and phone='+915800000601'$$,$$select 1$$,'create mode invents no duplicate member');
...
select results_eq($$select count(*) from public.members where tenant_id='58000000-0000-4000-8000-000000000001' and phone='+915800000031'$$,$$select 1$$,'a duplicate refusal creates no member');
...
select results_eq($$select x.r->>'memberUnavailable', (select count(*) from jsonb_object_keys(x.r)), x.r->>'memberId', x.r->>'fullName' from (values (public.convert_lead('58000000-0000-4000-8000-000000000603','58000000-0000-4000-8000-000000000705',(select revision from lead_fix where label='lc3'),'create',null)),(public.convert_lead('58000000-0000-4000-8000-000000000607','58000000-0000-4000-8000-000000000706',(select revision from lead_fix where label='lc7'),'create',null)),(public.convert_lead('58000000-0000-4000-8000-000000000608','58000000-0000-4000-8000-000000000707',(select revision from lead_fix where label='lc8'),'create',null))) x(r)$$,$$select 'true',1,null,null from generate_series(1,3)$$,'cancelled, blocked and erased duplicates all return a bare member-unavailable conflict');
...
select results_eq($$select (select count(*) from public.members where phone='+915800000037' and tenant_id='58000000-0000-4000-8000-000000000001'), (select count(*) from public.members where phone='+915800000037' and tenant_id='58000000-0000-4000-8000-000000000002')$$,$$select 1,1$$,'the cross-gym phone becomes this gym member and leaves the other gym untouched');
...
select results_eq($$select count(*) from public.leads where converted_member_id='58000000-0000-4000-8000-000000000031'$$,$$select 1$$,'only the accepted link connected the member');
...
select results_eq($$select count(*) from public.members where phone='+915800000625'$$,$$select 0$$,'no member row survives the rollback');
...
select is((select count(*) from public.audit_log), 0::bigint, 'lead conversions invent no audit rows');
