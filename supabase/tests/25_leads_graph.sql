...
select results_eq($$select stage::text, full_name, phone, email, source::text, assigned_to_staff_id::text, trial_at is null, converted_member_id is null, converted_at is null, lost_reason is null, created_by_staff_id::text, creation_request_key::text, revision is not null from public.leads where id=(select (result->>'leadId')::uuid from lead_rpc where label='first')$$,$$select 'new','Rahul Sharma','+919876543210','rahul@example.com','walk_in','57000000-0000-4000-8000-000000000024',true,true,true,true,'57000000-0000-4000-8000-000000000023','57000000-0000-4000-8000-000000000501',true$$,'a new enquiry row carries normalized facts, a null stage payload and stamped evidence');
...
select results_eq($$select count(*) from public.leads where tenant_id='57000000-0000-4000-8000-000000000001' and phone='+919876543210'$$,$$select 2::bigint$$,'the replay created no extra row while the fresh key created exactly one');
...
create temp table pre_sched_probe as select revision from public.leads where id=(select (result->>'leadId')::uuid from lead_rpc where label='l2');
...
select results_eq($$select stage::text, trial_at='2026-09-20 04:30:00+00'::timestamptz, lost_reason is null, revision::text<>(select revision::text from pre_sched_probe) from public.leads where id=(select (result->>'leadId')::uuid from lead_rpc where label='l2')$$,$$select 'trial_scheduled',true,true,true$$,'trial_scheduled stores the exact instant and rotates the revision');
...
select throws_ok($$select public.transition_lead((select (result->>'leadId')::uuid from lead_rpc where label='l2'),(select revision from public.leads where id=(select (result->>'leadId')::uuid from lead_rpc where label='l2')),'lost',null,'   ')$$,'GL060',null,'lost with a whitespace-only reason is an invalid loss fact');
...
select results_eq($$select full_name, stage::text, revision::text<>(select revision::text from direct_probe), updated_at>=(select updated_at from direct_probe) from public.leads where id=(select (result->>'leadId')::uuid from lead_rpc where label='l5')$$,$$select 'Meera Khan Edited','new',true,true$$,'a direct material edit also rotates revision and bumps updated_at no earlier than the probe');
...
select throws_ok($$update public.leads set notes='Preview edit'$$,'42501','Support preview is read-only.','a preview identity cannot mutate lead rows');
...
set local role postgres;
select is((select count(*) from public.audit_log where tenant_id in ('57000000-0000-4000-8000-000000000001','57000000-0000-4000-8000-000000000002')), 0::bigint, 'lead writes invent no audit rows');
