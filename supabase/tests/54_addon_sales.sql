...
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),'54000000-0000-4000-8000-000000000025',null,null,'cash','Paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay trainer is idempotency_conflict');
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),null,transaction_timestamp(),null,'cash','Paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay slot start is idempotency_conflict');
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),null,null,transaction_timestamp(),'cash','Paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay slot end is idempotency_conflict');
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),null,null,null,'upi','Paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay method is idempotency_conflict');
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),null,null,null,'cash','paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay reason case is idempotency_conflict');
...
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000101',2,(select (sale_request->>'quoteVersion')::uuid from public.addon_orders o join sale_results x using(order_id) where x.label='product-first'),null,null,null,'cash','Paid  CAFÉ','54000000-0000-4000-8000-000000000201')$$,'GL052',null,'changed replay seller is idempotency_conflict');
...
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000107',1,(select quote_version from public.addon_products where id='54000000-0000-4000-8000-000000000107'),null,null,null,'cash',null,'54000000-0000-4000-8000-000000000208')$$,'P0002',null,'foreign offer is indistinguishable from missing');
...
select results_eq($$select r.status::text,r.amount_paise,r.currency,r.reason,r.processed_at from public.refunds r join refund_results x using(refund_id) where x.label='partial-request'$$,$$select 'requested'::text,20000::bigint,'INR'::text,'Partial  PT  return'::text,null::timestamptz$$,'refund request is reserved headroom rather than returned cash');
...
set local role postgres;
select set_config('request.jwt.claims','',true);
create temp table refund_replay_before as select (select to_jsonb(r) from public.refunds r join refund_results x using(refund_id) where x.label='partial-request') refund_row,(select to_jsonb(o) from public.addon_orders o where id=(select order_id from sale_results where label='pt-first')) order_row,(select count(*) from public.audit_log where tenant_id='54000000-0000-4000-8000-000000000001') audits;
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"54000000-0000-4000-8000-000000000901","role":"authenticated","app_role":"gym_owner","tenant_id":"54000000-0000-4000-8000-000000000001","staff_id":"54000000-0000-4000-8000-000000000021"}',true);
...
set local role postgres;
select set_config('request.jwt.claims','',true);
select results_eq($$select (select to_jsonb(r) from public.refunds r join refund_results x using(refund_id) where x.label='partial-request'),(select to_jsonb(o) from public.addon_orders o where id=(select order_id from sale_results where label='pt-first')),(select count(*) from public.audit_log where tenant_id='54000000-0000-4000-8000-000000000001')$$,$$select refund_row,order_row,audits from refund_replay_before$$,'refund completion replay writes no refund, order or audit change');
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"54000000-0000-4000-8000-000000000901","role":"authenticated","app_role":"gym_owner","tenant_id":"54000000-0000-4000-8000-000000000001","staff_id":"54000000-0000-4000-8000-000000000021"}',true);
...
select throws_ok($$update public.payments set membership_id='54000000-0000-4000-8000-000000000999' where id=(select payment_id from sale_results where label='product-first')$$,'GL038',null,'arrived add-on payment cannot later buy a membership');
...
select results_eq($$select a.record_type,a.tenant_id,a.actor_user_id,a.actor_role::text,a.reason,a."before" is null,(select array_agg(k order by k) from jsonb_object_keys(a."after") k)=(select array_agg(column_name order by column_name) from information_schema.columns where table_schema='public' and table_name='addon_orders' and column_name not in ('id','tenant_id','created_at','updated_at')) from public.audit_log a join sale_results x on x.order_id=a.record_id where x.label='product-first' and a.action='addon_order.created'$$,$$select 'addon_order'::text,'54000000-0000-4000-8000-000000000001'::uuid,'54000000-0000-4000-8000-000000000903'::uuid,'front_desk'::text,'Paid  CAFÉ'::text,true,true$$,'created audit has exact actor, reason and every domain column');
select results_eq($$select count(*),bool_and((select array_agg(k order by k) from jsonb_object_keys(a."before") k)=(select array_agg(column_name order by column_name) from information_schema.columns where table_schema='public' and table_name='addon_orders' and column_name not in ('id','tenant_id','created_at','updated_at')) and (select array_agg(k order by k) from jsonb_object_keys(a."after") k)=(select array_agg(column_name order by column_name) from information_schema.columns where table_schema='public' and table_name='addon_orders' and column_name not in ('id','tenant_id','created_at','updated_at'))) from public.audit_log a join sale_results x on x.order_id=a.record_id where x.label='product-first' and a.action='addon_order.updated'$$,$$select 3::bigint,true$$,'product acceptance writes exact before/after summaries for each database-derived transition');
