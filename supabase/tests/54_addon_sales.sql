...
select throws_ok($$select * from public.schedule_pt_session((select order_id from sale_results where label='pt-first'),'54000000-0000-4000-8000-000000000301',(select starts_at+interval '1 minute' from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),(select ends_at from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),'Keep  form  notes')$$,'GL052',null,'same session UUID with changed slot is idempotency_conflict');
select throws_ok($$select * from public.schedule_pt_session((select order_id from sale_results where label='pt-first'),'54000000-0000-4000-8000-000000000301',(select starts_at from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),(select ends_at from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),'keep  form  notes')$$,'GL052',null,'same session UUID with changed note case is idempotency_conflict');
...
select throws_ok($$select * from public.finish_pt_session('54000000-0000-4000-8000-000000000303','completed')$$,'GL058',null,'a PT session cannot complete before its end');
...
select set_config('request.jwt.claims','{"sub":"54000000-0000-4000-8000-000000000905","role":"authenticated","app_role":"trainer","tenant_id":"54000000-0000-4000-8000-000000000001","staff_id":"54000000-0000-4000-8000-000000000025"}',true);
select throws_ok($$select * from public.record_addon_sale('54000000-0000-4000-8000-000000000031','54000000-0000-4000-8000-000000000102',1,(select quote_version from public.addon_products where id='54000000-0000-4000-8000-000000000102'),null,null,null,null,'Trainer cannot sell','54000000-0000-4000-8000-000000000230')$$,'42501',null,'trainer role cannot invoke front-office sale command');
select set_config('request.jwt.claims','{"sub":"54000000-0000-4000-8000-000000000903","role":"authenticated","app_role":"front_desk","tenant_id":"54000000-0000-4000-8000-000000000001","staff_id":"54000000-0000-4000-8000-000000000023"}',true);
...
select results_eq($$select * from pg_temp.captured_error($sql$select * from public.schedule_pt_session((select order_id from sale_results where label='pt-first'),'54000000-0000-4000-8000-000000000301',(select starts_at+interval '1 minute' from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),(select ends_at from public.pt_sessions where id='54000000-0000-4000-8000-000000000301'),'Keep  form  notes')$sql$)$$,$$values ('GL052'::text,'idempotency_conflict'::text)$$,'GL052 identifies changed session creation replay exactly');
select set_config('request.jwt.claims','{"sub":"54000000-0000-4000-8000-000000000903","role":"authenticated","app_role":"front_desk","tenant_id":"54000000-0000-4000-8000-000000000001","staff_id":"54000000-0000-4000-8000-000000000023"}',true);
select results_eq($$select * from pg_temp.captured_error($sql$insert into public.pt_sessions(id,tenant_id,addon_order_id,trainer_staff_id,member_id,starts_at,ends_at,status) values ('54000000-0000-4000-8000-000000000513','54000000-0000-4000-8000-000000000001','54000000-0000-4000-8000-000000000502','54000000-0000-4000-8000-000000000025','54000000-0000-4000-8000-000000000032',clock_timestamp()+interval '3 hours',clock_timestamp()+interval '4 hours','scheduled')$sql$)$$,$$values ('GL058'::text,'session_identity_mismatch'::text)$$,'GL058 identifies a direct member/order identity mismatch exactly');
