...
-- TWENTY-THIRD-ROUND EXTENSION — Section 37 (assertions 783-812), contract
-- `membership-creation` / GL048, written from that contract ALONE by an
-- implementation-blind author: no migration was opened, no function or trigger
-- definition was read, and docs/decisions.md, docs/registry.md and
-- docs/data-model.md were not opened either. Table shape came from
-- information_schema and pg_constraint; house style from
-- 21_manual_payment.sql and this file's own earlier sections.
--
-- It closes the door twenty-two rounds of UPDATE rules never looked at — the
-- INSERT, where the price, the length, the count and the dates all first
-- arrive — and the one place a bought period gets added to a typed one.
-- Section 37's own header carries the attack list, the ordering the contract
-- decides (GL048 ahead of GL046) and the one it deliberately does not
-- (GL048 against GL044, pinned as undecided at 802 per ADR-100).
--
-- PLAN COUNT: 812 (782 + 30). COMMITTED RED: ten of the thirty are expected to
-- fail against live Cloud today, because the rule they assert does not exist
-- yet — 784/785 and 792/793/794 (an over-long creation and both half-dated
-- creations all land), 798/799/801 (GL046 answers where GL048 should, and the
-- over-long creation at the plan's own price is refused by nothing at all), and
-- 807/808 (the doubling itself: one period's money buys sixty days). The other
-- twenty are green today and must STAY green: they are the permitted shapes
-- (786-791), the table's own CHECKs (795-797), GL044 (802/803), GL046 still
-- answering where it should (800), the ordinary dateless path (809/810) and the
-- renewal (811/812) — which together are what a too-broad fix breaks and
...
-- ===========================================================================
-- SECTION 37 (ROUND TWENTY-THREE, `membership-creation` / GL048) — WHAT A
-- MEMBERSHIP MAY BE CREATED **AS**, and the first period being SET rather than
-- ADDED. Tenant 30. Assertions 783-812.
--
-- WHY THIS SECTION EXISTS. Every round from nine to twenty-two governed an
-- UPDATE to a membership — the price (GL046), the length (ADR-092), the count
-- (GL044), the dates (GL045) — and not one of them looked at the INSERT, which
-- is where all four of those values first arrive. The contract this section is
-- written from names two rules and this file asserts both:
--
--   1. A creation is refused unless `ends_on` is at most `starts_on` plus the
--      plan's duration, or both dates are null.  Code `GL048`.
--   2. WHERE a membership has been granted no periods, the granting rule SETS
--      its span from the plan rather than EXTENDING a span it already carries.
--
-- THE MEASURED DEFECT REQUIREMENT 2 CLOSES, and it is the reason this section
-- exists at all. An ordinary front desk, two ordinary statements, no privilege
-- beyond selling and taking money:
--
--     insert into public.memberships (… starts_on, ends_on …)
--       values (… today, today + 30 …);        -- a 30-day plan, legal span
--     insert into public.payments (… 100000, 'paid', 'cash' …);  -- one period
--
--     ends_on - starts_on = 60   periods_granted = 1
--
-- Sixty days for one period's money, and EVERY audit invariant intact: one
-- payment, one receipt, `periods_granted = floor(money / price)`, the plan's own
-- price on the row, no frozen term touched. The product's own path — the
-- dateless `pending` membership the console writes and the money dates — gives
-- 30 for the same payment. Nothing in this system compares `ends_on - starts_on`
-- against `duration_days × periods_granted` (ADR-088 declined to add exactly
-- that invariant), so the doubled row audits clean. That is assertions 806-808,
-- and they are the most important ones here.
--
-- WHY TWO RULES AND NOT ONE, since requirement 1 already bounds a typed span.
-- Because the two are keyed on different things and one of them can be switched
-- off. Requirement 1 belongs beside `GL044` in the membership-terms trigger,
-- which BOTH seed files disable around their own statements — so it is off for
-- the seed, exactly as `GL044` and `GL042` already are. Requirement 2 lives in
-- the granting rule itself, which no `disable trigger` can reach. Requirement 1
-- is keyed on what a writer may TYPE; requirement 2 on what has actually been
-- PAID FOR. ADR-089's general form applied twice: key two rules on two different
-- things, because a rule is worth exactly what the immutability of the thing it
-- is keyed on is worth.
--
-- THE ORDERING, TAKEN FROM THE CONTRACT RATHER THAN GUESSED (37c and 37d).
--   * `GL048` answers ahead of `GL046`, on the precedent already recorded for
--     `GL043` and re-argued in Section 20's own header: an absolute beats a
--     permission, and answering the permission would tell a desk that a gym
--     admin could still do it, which is false. 798 is that assertion, and 799
--     and 800 are its two controls — they are not decoration, see 37c.
--   * `GL048` against `GL044` is DELIBERATELY NOT DECIDED by the contract,
--     because no scenario needs it. Per ADR-100 an undecided pair gets an
--     assertion pinning that nobody may rely on it rather than silence, so 802
--     asserts the refusal with `null::char(5)` and NOT a code. Any future
--     assertion that pins one is asserting something the contract does not say.
--
-- AND NOTHING HERE ANSWERS AHEAD OF THE TABLE'S OWN SHAPE (37b). A CHECK is
-- evaluated against the tuple being stored, before index insertion and before
-- every AFTER trigger, so where one bites it beats this rule too. 795 and 796
-- pin the two shapes that reach a CHECK instead of `GL048` —
-- `memberships_ends_on_after_starts_on_chk` and
-- `memberships_dated_unless_pending_chk`, both `23514`. Neither is a `GL048`
-- violation in the first place (a backwards span is trivially "at most" one
-- period, and both-dates-null is expressly permitted), so there is no ordering
-- question hidden in them — they are the guard the sibling requirement lost two
-- rounds to leaving off, written here first, and they are also why every
-- half-dated fixture below is `pending`: an `active` half-dated row would die of
-- the CHECK and prove nothing about `GL048`.
--
-- ZERO SPAN IS ALLOWED AND THAT IS NOT A CONCESSION (788). `POST /api/
-- memberships` writes `status = 'active'`, `starts_on = today`,
-- `ends_on = today`, and ADR-083 records why: the line used to ADD the plan's
-- duration, and "a desk that sold a membership and then took the money for it
-- granted sixty days for one month's fee, in two clicks that both looked right."
-- The screen was corrected. The table was not, which is the oldest shape in this
-- codebase and is what requirement 2 moves to where a hand-written statement has
-- to obey it as well.
--
-- ONE PERIOD, NOT ZERO SPAN, IS THE CREATION RULE, and the contract measured
-- rather than argued it: of 46 live memberships, 45 span exactly one period, one
-- has both dates null, none is half-dated, and none would be refused — while 15
-- of the dated ones carry no payment at all and are the lapsed/expired/cancelled
-- fixtures the whole retention loop is demonstrated on. "No span at all" would
-- force a seed rewrite or a trusted-caller carve-out; "at most one period" needs
-- neither, and turns the measured exploit (`today … today + 3650`, ten years
-- bought with nothing, live at the turnstile the same day because the check-in
-- gate reads dates) into one month — the length the gym sells anyway.
--
-- FIXTURES. FOURTEEN members, one per creation attempted, so that a statement
-- which unexpectedly LANDS cannot leave the next attempt's member holding
-- something and turn its assertion green — or red — for the wrong reason. That
-- matters more here than usual: several of these are expected to land today, and
-- a landed `active` row would collide with the live partial unique index and
-- answer `23505` where the section means to read `GL048`. Every refusal is
-- followed by a results_eq re-reading the table, because throws_ok proves an
-- exception was raised and not that nothing was written. The ordinary-path
-- membership (94) is INSERTED as a trusted write rather than earned, because
-- creation is not what requirement 2 governs and staging it through the rule
-- under test would stage nothing.
--
-- ADR-039: every date is the gym's own `(now() at time zone o.timezone)::date`
-- via `today_t30`, never `current_date` and never a literal. ADR-030: nothing is
-- committed; the file's single BEGIN … ROLLBACK covers it. ADR-050: no assertion
-- here depends on another tenant's rows.
-- ===========================================================================

set local role postgres;

insert into public.organizations (id, name, gym_code) values
  ('22000000-0000-4000-8000-000000300001'::uuid, 'PayRec Gym 30', 'PYR230');
...
-- One member per creation attempted. 41-52 are the twelve creations of 37a-37d
-- (five of which are meant to land and seven of which are meant to be refused);
-- 53 is the doubling; 54 is the ordinary dateless path and the renewal that
-- follows it.
insert into public.members (id, tenant_id, branch_id, full_name, phone) values
  ('22000000-0000-4000-8000-000000300041'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 OverLong', '+912230000041'),
  ('22000000-0000-4000-8000-000000300042'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 OnePeriod', '+912230000042'),
  ('22000000-0000-4000-8000-000000300043'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 ZeroSpan', '+912230000043'),
  ('22000000-0000-4000-8000-000000300044'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 NoDates', '+912230000044'),
  ('22000000-0000-4000-8000-000000300045'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 HalfStart', '+912230000045'),
  ('22000000-0000-4000-8000-000000300046'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 HalfEnd', '+912230000046'),
  ('22000000-0000-4000-8000-000000300047'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 Backwards', '+912230000047'),
  ('22000000-0000-4000-8000-000000300048'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 ActiveNull', '+912230000048'),
  ('22000000-0000-4000-8000-000000300049'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 LongWrongPrice', '+912230000049'),
  ('22000000-0000-4000-8000-000000300050'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 LongListPrice', '+912230000050'),
  ('22000000-0000-4000-8000-000000300051'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 FitWrongPrice', '+912230000051'),
  ('22000000-0000-4000-8000-000000300052'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 LongAndCounted', '+912230000052'),
  ('22000000-0000-4000-8000-000000300053'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 Doubling', '+912230000053'),
  ('22000000-0000-4000-8000-000000300054'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300011'::uuid, 'M30 Ordinary', '+912230000054');

-- One plan, 30 days, Rs 1,000. Every expected value below is this plan's
-- duration or this plan's price, never a bare literal standing in for one.
insert into public.plans (id, tenant_id, name, duration_days, price_paise) values
...
-- Membership 94: the ordinary path's own shape — `pending`, both dates null, at
-- the plan's price, granted nothing. This is what the product writes when a
-- membership is sold before the money arrives, and what 809-812 pay against.
-- Written as a trusted insert: creation is not what requirement 2 governs, and
-- staging a fixture through the rule under test stages nothing.
insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise) values
  ('22000000-0000-4000-8000-000000300094'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
   '22000000-0000-4000-8000-000000300054'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
   'pending', null, null, 100000);

-- 783 — the fixture pinned before anything is attempted, because every expected
-- value below is derived from one of these: the plan is 30 days at Rs 1,000 (so
-- `today + 30` really is one period and `today + 31` really is more than one),
-- membership 94 is pending, null-dated and granted nothing (so 810 measures a
-- FIRST grant and not a second), and tenant 30 holds exactly that one membership
-- and no payment at all (so no count below can be answered by a stray row).
-- Verifying a no-op is not verifying.
select results_eq(
  $$ select (select p.duration_days::int from public.plans p where p.id = '22000000-0000-4000-8000-000000300061'::uuid),
            (select p.price_paise::bigint from public.plans p where p.id = '22000000-0000-4000-8000-000000300061'::uuid),
            (select m.status::text from public.memberships m where m.id = '22000000-0000-4000-8000-000000300094'::uuid),
            (select (m.starts_on is null and m.ends_on is null) from public.memberships m where m.id = '22000000-0000-4000-8000-000000300094'::uuid),
            (select m.periods_granted from public.memberships m where m.id = '22000000-0000-4000-8000-000000300094'::uuid),
            (select count(*)::int from public.memberships m where m.tenant_id = '22000000-0000-4000-8000-000000300001'::uuid),
            (select count(*)::int from public.payments p where p.tenant_id = '22000000-0000-4000-8000-000000300001'::uuid) $$,
  $$ values (30, 100000::bigint, 'pending'::text, true, 0, 1, 0) $$,
  'guarded fixture: the plan is 30 days at Rs 1,000, membership 94 is pending with BOTH dates null and NOTHING granted, and tenant 30 holds exactly that one membership and no payment'
);

-- The claim is set once for the whole section: `set_config(..., true)` is
-- transaction-local and `set local role` touches only `role`, so every
-- `set local role authenticated` below re-enters this same front-desk session.
-- Front desk deliberately: it is the role the contract measured both exploits
-- from, and it is the role a permission rule could otherwise mask.
select set_config(
  'request.jwt.claims',
  json_build_object('sub', gen_random_uuid(), 'role', 'authenticated',
...
-- ---------------------------------------------------------------------------
-- 37a (784-794) — THE SHAPE OF A CREATION. Five shapes, and the rule's whole
-- content is which side of the line each falls on: more than one period is
-- refused; exactly one period, zero span and both-dates-null are allowed; one
-- date without the other is refused. The three permitted ones are asserted
-- because a rule that refuses everything passes every refusal assertion ever
-- written, and this project has shipped that too-broad shape three times in this
-- phase alone. Every one of them is what something real already writes: exactly
-- one period is the shape all 45 dated memberships in the database have, zero
-- span is what `POST /api/memberships` writes, and both-null is what a
-- membership looks like between being sold and being paid for.
-- ---------------------------------------------------------------------------

set local role authenticated;

-- 784 — the rule itself. `starts_on … starts_on + duration + 1`: one day more
-- than the plan sells, at the plan's own price, from an ordinary front desk in
-- one statement. The contract's measured exploit is this shape at `+ 3650`; a
-- rule bounded at the plan's duration turns that into a rounding, and this is
-- the first day on the wrong side of the bound.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300081'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300041'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 31, 100000)
$$, 'GL048'::char(5), null,
  'GL048: a membership created spanning ONE DAY more than the plan it names is refused — the dates are what the money bought, and no money has bought anything here');

set local role postgres;

-- 785 — refused AND nothing landed. throws_ok proves an exception was raised,
-- not that no row was written, and a row sitting there dated ten years out is
-- the entire harm: the check-in gate reads dates, so the member is live at the
-- turnstile today, and GL045 then refuses every attempt to correct the row.
select results_eq(
  $$ select count(*)::int from public.memberships
      where id = '22000000-0000-4000-8000-000000300081'::uuid $$,
  $$ values (0) $$,
  'GL048: and NO membership exists afterwards — nothing is sitting there live at the turnstile waiting for GL045 to make it permanent'
);

set local role authenticated;

-- 786 — exactly one period. The shape every seeded fixture already has: all 45
-- dated memberships in the database span exactly this, so a rule that refused it
-- would put the seed red and would be the "no span at all" rule the contract
-- measured and rejected.
select lives_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300082'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
...
$$, 'a membership created spanning EXACTLY the plan''s duration is allowed — the shape all 45 dated memberships in the database have, and the boundary is inclusive');

set local role postgres;

-- 787
select results_eq(
  $$ select status::text, starts_on, ends_on, periods_granted from public.memberships
      where id = '22000000-0000-4000-8000-000000300082'::uuid $$,
  $$ select 'active'::text, (select d from today_t30), (select d from today_t30) + 30, 0 $$,
  'and it LANDED as named — active, one period long, and granted nothing, which is what makes it the doubling''s own fixture shape'
);

set local role authenticated;

-- 788 — zero span, `active`, both dates today: what `POST /api/memberships`
-- writes today, argued in the handler itself (PAY-011 needs a cash-only gym
-- fully functional, and "a membership nobody can check in against is not
-- that"). A rule refusing this breaks the console on its first click.
select lives_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300083'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
...
$$, 'a membership created with ZERO span — both dates today — is allowed: this is exactly what the console writes, and ADR-083 records what happened the one time that line added the plan''s duration instead');
...
select results_eq(
  ...
  $$ select 'active'::text, (select d from today_t30), (select d from today_t30), 0 $$,
  'and it LANDED active with both dates today — the caller asked for active and got active, which PAY-011 requires of a gym with no gateway'
);

set local role authenticated;

-- 790 — both dates null, `pending`: permitted by
-- memberships_dated_unless_pending_chk, and the granting rule has a whole branch
-- that dates such a row from the plan. This requirement neither adds that shape
-- nor removes it, and 809-810 pay against it.
select lives_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, price_paise)
  values ('22000000-0000-4000-8000-000000300084'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300044'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'pending', 100000)
$$, 'a PENDING membership created naming neither date is allowed, exactly as it is today — the product''s own sold-but-not-yet-paid shape');
...
select results_eq(
  ...
  $$ select 'pending'::text, null::date, null::date, 0 $$,
  'and it LANDED pending with both dates still null — the rule''s "or both dates are null" branch is a permission and not merely an absence of refusal'
);

set local role authenticated;

-- 792 — half-dated, `starts_on` set and `ends_on` null. Refused, and that is a
-- deliberate consequence of the wording rather than an accident of it: it is
-- half of OPEN-026, whose whole subject is that nothing in the product can
-- create such a row and nothing can repair one. `pending` deliberately — an
-- `active` half-dated row dies of the CHECK at 796 and would prove nothing here.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, price_paise)
  values ('22000000-0000-4000-8000-000000300085'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300045'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'pending', (select d from today_t30), 100000)
$$, 'GL048'::char(5), null,
  'GL048: a half-dated creation — starts_on set, ends_on null — is refused. It is neither "at most one period" nor "both dates null", and it is the OPEN-026 shape nothing in the product can create or repair');

-- 793 — the other half of the same shape, because a rule reading only
-- `ends_on is null` and a rule reading "both or neither" pass and fail
-- differently here, and only one of them is the contract's.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300086'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300046'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'pending', (select d from today_t30) + 30, 100000)
$$, 'GL048'::char(5), null,
  'GL048: and the mirror — ends_on set, starts_on null — is refused too, so the rule is "both or neither" and not a one-sided null check');
...
select results_eq(
  $$ select count(*)::int from public.memberships
      where id in ('22000000-0000-4000-8000-000000300085'::uuid,
                   '22000000-0000-4000-8000-000000300086'::uuid) $$,
  $$ values (0) $$,
  'GL048: and NEITHER half-dated row landed — OPEN-026''s creation half is closed at the door rather than left to be discovered on a row nobody can fix'
);

-- ---------------------------------------------------------------------------
-- 37b (795-797) — NOTHING ANSWERS AHEAD OF THE TABLE'S OWN SHAPE. A CHECK is
-- evaluated against the tuple being stored, before index insertion and before
-- every AFTER trigger, so where one bites it beats this rule as it beats every
-- other. Neither shape here is a GL048 violation in the first place — a
-- backwards span is trivially "at most" one period, and both-dates-null is
-- expressly permitted — so there is no ordering question smuggled into these
-- two: they pin which mechanism owns which shape, and they are why every
-- half-dated fixture above is `pending`. The sibling requirement lost two rounds
-- to leaving this guard off.
-- ---------------------------------------------------------------------------
...
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300087'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300047'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) - 1, 100000)
$$, '23514'::char(5), null,
  'a creation whose ends_on PRECEDES its starts_on is 23514 from memberships_ends_on_after_starts_on_chk, before any after trigger — a negative span is the table''s business, not this rule''s, and this rule would have permitted it');

-- 796
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, price_paise)
  values ('22000000-0000-4000-8000-000000300088'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300048'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', 100000)
$$, '23514'::char(5), null,
  'and an ACTIVE creation with null dates is 23514 from memberships_dated_unless_pending_chk — the shape 790 is allowed to have only because it is pending, which is why every half-dated fixture above is pending too');
...
select results_eq(
  $$ select count(*)::int from public.memberships
      where id in ('22000000-0000-4000-8000-000000300087'::uuid,
                   '22000000-0000-4000-8000-000000300088'::uuid) $$,
  $$ values (0) $$,
  'and neither landed — the guards refuse as well as raise, so 795/796 are not two statements that quietly wrote something on their way to an error'
);

-- ---------------------------------------------------------------------------
-- 37c (798-801) — `GL048` ANSWERS AHEAD OF `GL046`. An absolute beats a
-- permission: answering `GL046` would tell a desk that a gym admin could still
-- create a ten-year membership, and they cannot. This is the precedent already
-- recorded for `GL043` and re-argued in Section 20's header.
--
-- THE THREE ARE A CONTROLLED TRIPLE and the two controls are load-bearing. 798
-- varies both things at once (over-long AND a price the plan does not carry) and
-- so on its own cannot say WHICH rule replied. 799 holds the price at the
-- plan's and keeps the over-long span — `GL048` — which says the span alone is
-- sufficient. 800 holds the span at exactly one period and keeps the wrong price
-- — `GL046` — which says the price alone is ALSO sufficient in this same
-- session, so 798's `GL048` is a genuine ordering and not `GL046` being absent
-- or unreachable here. Without 800 a fix that simply stopped enforcing `GL046`
-- at creation would pass 798 and 799 and this whole group would mean nothing.
-- Assertion 799 deliberately repeats 784's shape inside the group rather than
-- referring to it, so that the triple varies exactly one thing at a time; the fourth cell of
-- the square — right span, right price — is 786.
-- ---------------------------------------------------------------------------

set local role authenticated;

-- 798 — THE ORDERING ASSERTION.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300089'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300049'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 31, 15000)
$$, 'GL048'::char(5), null,
  'GL048 ahead of GL046: a front desk creating an OVER-LONG membership at a tenth of the plan''s price answers the SPAN rule, not the permission rule — an absolute beats a permission, and GL046 here would imply a gym admin could do it, which they cannot');

-- 799 — CONTROL: the same over-long creation at the plan's OWN price. If this
-- did not answer GL048, 798's code would be telling us about the price and not
-- about the span.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300090'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300050'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 31, 100000)
$$, 'GL048'::char(5), null,
  'CONTROL: the SAME over-long creation at the plan''s own price is GL048 as well — the span alone is sufficient, so 798 is not GL048 arriving by way of the price');

-- 800 — CONTROL: the same wrong price at a span of exactly one period. This is
-- the one that makes 798 mean something: GL046 is live, reachable and correct in
-- this very session, and it loses to GL048 only because the span is illegal.
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300091'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300051'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 30, 15000)
$$, 'GL046'::char(5), null,
  'CONTROL: the same tenth-of-list price at a CORRECTLY SIZED span is GL046 — the permission rule is live and reachable in this exact session, which is the only thing that makes 798''s GL048 an ordering rather than an absence');
...
select results_eq(
  $$ select count(*)::int from public.memberships
      where id in ('22000000-0000-4000-8000-000000300089'::uuid,
                   '22000000-0000-4000-8000-000000300090'::uuid,
                   '22000000-0000-4000-8000-000000300091'::uuid) $$,
  $$ values (0) $$,
  'and none of the three landed — whichever rule answered, nothing is sitting there waiting for a payment to be scored against it'
);

-- ---------------------------------------------------------------------------
-- 37d (802-803) — `GL048` AGAINST `GL044` IS DELIBERATELY UNDECIDED. One
-- statement violates both: it spans more than the plan sells AND it names a
-- non-zero `periods_granted`, which is "a membership is created having been
-- granted nothing". The contract states in as many words that it does not decide
-- which of the two answers, because no scenario needs it — and per ADR-100 an
-- undecided pair gets an assertion pinning that nobody may rely on it rather
-- than silence. So this asserts ONLY that it is refused and that nothing landed.
--
-- NOBODY MAY RELY ON WHICH CODE COMES BACK HERE. A future assertion that pins
-- one is asserting something the contract does not say, and an implementation is
-- free to answer either without breaking this suite. That is the point of the
-- `null::char(5)`: it is a deliberate absence, not an author who could not be
-- bothered to look one up.
-- ---------------------------------------------------------------------------
...
select throws_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise, periods_granted)
  values ('22000000-0000-4000-8000-000000300092'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300052'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 31, 100000, 3)
$$, null::char(5), null,
  'over-long AND carrying a typed periods_granted in ONE statement is refused — and the code is deliberately NOT pinned: the contract does not decide GL048 against GL044, so nobody may rely on which answers');
...
select results_eq(
  $$ select count(*)::int from public.memberships
      where id = '22000000-0000-4000-8000-000000300092'::uuid $$,
  $$ values (0) $$,
  'and no row landed — the outcome both rules agree on is the one this asserts, which is the whole of what the contract commits to here'
);

-- ---------------------------------------------------------------------------
-- 37e (804-808) — THE DOUBLING. REQUIREMENT 2, AND THE MOST IMPORTANT
-- ASSERTIONS IN THIS SECTION.
--
-- Two ordinary front-desk statements, both individually legal, both individually
-- correct, and neither of them touching a frozen term: create a membership dated
-- exactly one period (which 786 has just proved is permitted and which all 45
-- dated memberships in the database look like), then record ONE payment of the
-- plan's own price. Measured today:
--
--     ends_on - starts_on = 60   periods_granted = 1
--
-- Sixty days for one period's money. `app.grant_periods()` computes
-- `ends_on = greatest(ends_on, today) + duration × periods`, which it MUST do
-- for renewals — but when `periods_granted = 0` any span the row carries was
-- TYPED and not BOUGHT, and adding a bought period on top of a typed one hands
-- the typed one out free. The product's own dateless path (809-810) gives 30 for
-- the identical payment.
--
-- AND IT AUDITS CLEAN, which is why no existing assertion in this file catches
-- it: one payment, one receipt, the plan's own price on the row,
-- `periods_granted = floor(money / price)` holding exactly, no frozen term
-- moved. The ONLY disagreement is `ends_on - starts_on` against
-- `duration_days × periods_granted`, and nothing in this system compares those
-- two numbers — ADR-088 declined to add exactly that invariant as a trigger, for
-- reasons that are still right. So 807 and 808 assert the one thing a clean
-- audit cannot: that a period's money bought a period's days.
--
-- Assertion 805 is the guard that makes 807 mean something. Without it, 807
-- passing could mean the membership never carried a typed span at all, and the doubling would
-- be untested while reporting green.
-- ---------------------------------------------------------------------------

set local role authenticated;

-- 804 — the exploit's own first statement, from an ordinary front desk.
select lives_ok($$
  insert into public.memberships (id, tenant_id, member_id, plan_id, status, starts_on, ends_on, price_paise)
  values ('22000000-0000-4000-8000-000000300093'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300053'::uuid, '22000000-0000-4000-8000-000000300061'::uuid,
          'active', (select d from today_t30), (select d from today_t30) + 30, 100000)
$$, 'the doubling''s first statement: a front desk creates a membership dated exactly one period, at the plan''s own price. Permitted — by 786, by the seed, and by the rule above');

set local role postgres;

-- 805 — the guard. Nothing has been bought yet: a full period of days sits on
-- the row and NO money has arrived against it. This is the state requirement 2
-- is keyed on, and without pinning it 807 could pass for the wrong reason.
select results_eq(
  $$ select (m.ends_on - m.starts_on)::int, m.periods_granted,
            (select count(*)::int from public.payments p where p.membership_id = m.id)
       from public.memberships m where m.id = '22000000-0000-4000-8000-000000300093'::uuid $$,
  $$ values (30, 0, 0) $$,
  'guard: the row carries a full period of DAYS and has been granted NOTHING and taken NO money — the span on it was typed, not bought, which is exactly the state the granting rule must SET rather than extend'
);

set local role authenticated;

-- 806 — the exploit's own second statement. One payment, the plan's own price,
-- ordinary cash at the desk. Nothing about it is irregular and nothing about it
-- should be refused; the defect is in what it grants, not in whether it lands.
select lives_ok($$
  insert into public.payments (id, tenant_id, member_id, membership_id, amount_paise, currency, status, method, recorded_by_staff_id)
  values ('22000000-0000-4000-8000-0000003000a1'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300053'::uuid, '22000000-0000-4000-8000-000000300093'::uuid,
          100000, 'INR', 'paid', 'cash', '22000000-0000-4000-8000-000000300021'::uuid)
$$, 'the doubling''s second statement: ONE payment of exactly the plan''s price is recorded, not refused — one period''s money');

set local role postgres;

-- 807 — THE ASSERTION THIS WHOLE SECTION EXISTS FOR. The first grant SETS the
-- span from the plan; it does not add a period to a span the row already
-- carried. `starts_on … starts_on + duration`, one period granted, still active.
-- Today this reads `today + 60` and fails, which is the defect.
select results_eq(
  $$ select status::text, starts_on, ends_on, periods_granted from public.memberships
      where id = '22000000-0000-4000-8000-000000300093'::uuid $$,
  $$ select 'active'::text, (select d from today_t30), (select d from today_t30) + 30, 1 $$,
  'THE DOUBLING: one period''s money bought EXACTLY one period of days — the first grant SET the span from the plan instead of adding a bought period on top of a typed one. Today this row runs to today + 60'
);

-- 808 — the same claim stated as the number the audit trail cannot see:
-- `ends_on - starts_on` against `duration_days × periods_granted`. 30, not 60.
-- Written separately and not folded into 807 because this is the disagreement
-- ADR-088 declined to enforce as a trigger, and a reader chasing the defect
-- should find it as a single number rather than inside a four-column tuple.
select is(
  (select (m.ends_on - m.starts_on)::int from public.memberships m
    where m.id = '22000000-0000-4000-8000-000000300093'::uuid),
  (select (p.duration_days * m.periods_granted)::int from public.memberships m
     join public.plans p on p.id = m.plan_id
    where m.id = '22000000-0000-4000-8000-000000300093'::uuid),
  'and the span equals duration_days x periods_granted exactly — the one invariant nothing in this system compares, which is why the doubled row audits clean today'
);

-- ---------------------------------------------------------------------------
-- 37f (809-812) — THE ORDINARY PATH IS UNCHANGED, AND A RENEWAL STILL EXTENDS.
-- Requirement 2 is keyed on `periods_granted = 0` for exactly one reason: a
-- membership that HAS been granted a period must still extend, because extending
-- is what a renewal IS. A fix that reads "always set from the plan" passes 807
-- and 808 and breaks every renewal in the product — and nothing else in this
-- file would catch it, because no other section pays a membership twice through
-- the dateless door.
--
-- One membership walks both: 94 is the sold-but-unpaid shape the console writes,
-- dateless and pending. The first payment dates it from the plan and activates
-- it (810 — the ordinary path, which must read exactly as it does today). The
-- second payment is then a renewal against a membership that already holds a
-- period, and must ADD one (812). Same row, same price, same desk; the only
-- variable between the two is whether anything had been granted yet, which is
-- the rule's own key.
-- ---------------------------------------------------------------------------
...
select lives_ok($$
  insert into public.payments (id, tenant_id, member_id, membership_id, amount_paise, currency, status, method, recorded_by_staff_id)
  values ('22000000-0000-4000-8000-0000003000a2'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300054'::uuid, '22000000-0000-4000-8000-000000300094'::uuid,
          100000, 'INR', 'paid', 'cash', '22000000-0000-4000-8000-000000300021'::uuid)
$$, 'the ordinary path: one payment of the plan''s price against the dateless PENDING membership the product itself writes');

set local role postgres;

-- 810 — the ordinary path, unchanged. This is the number the doubling is
-- measured against: the identical payment, against the product's own shape,
-- buys 30 days and not 60.
select results_eq(
  $$ select status::text, starts_on, ends_on, periods_granted from public.memberships
      where id = '22000000-0000-4000-8000-000000300094'::uuid $$,
  $$ select 'active'::text, (select d from today_t30), (select d from today_t30) + 30, 1 $$,
  'the ordinary path is UNCHANGED — a dateless pending membership is dated from the plan, granted exactly one period and made active, exactly as it is today. This is the 30 the doubling''s 60 is measured against'
);
...
select lives_ok($$
  insert into public.payments (id, tenant_id, member_id, membership_id, amount_paise, currency, status, method, recorded_by_staff_id)
  values ('22000000-0000-4000-8000-0000003000a3'::uuid, '22000000-0000-4000-8000-000000300001'::uuid,
          '22000000-0000-4000-8000-000000300054'::uuid, '22000000-0000-4000-8000-000000300094'::uuid,
          100000, 'INR', 'paid', 'cash', '22000000-0000-4000-8000-000000300021'::uuid)
$$, 'the renewal: a second payment of the same price against the SAME membership, which now already holds a granted period');

set local role postgres;

-- 812 — and it EXTENDED. The span the row carries now was bought, so the second
-- period is added to it rather than replacing it: `today + 60`, two periods.
-- This is the assertion a too-broad fix fails, and it is why requirement 2 is
-- keyed on `periods_granted = 0` rather than on "the row has dates".
select results_eq(
  $$ select status::text, starts_on, ends_on, periods_granted from public.memberships
      where id = '22000000-0000-4000-8000-000000300094'::uuid $$,
  $$ select 'active'::text, (select d from today_t30), (select d from today_t30) + 60, 2 $$,
  'A RENEWAL STILL EXTENDS — the second period is ADDED to a span that was itself bought, giving today + 60 and two periods. A fix that always SET the span from the plan would pass 807/808 and silently cap every renewal in the product at one month'
);
