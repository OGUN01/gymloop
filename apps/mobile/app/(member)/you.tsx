...
export default function YouScreen() {
  ...
  return <Screen>
    ...
    {data ? <View style={styles.profile}><View style={[styles.avatar, { backgroundColor: palette.elevatedSurface }]}><Title>{initial}</Title></View><View style={styles.profileCopy}><Body>{data.member.fullName}</Body><Body muted>Verified member</Body><Eyebrow>{data.gym.name} · {data.gym.code}</Eyebrow><Body muted>{data.member.email ?? data.member.phone ?? 'Member account'}</Body></View></View> : null}
    <View role="list" accessibilityLabel="Account"><Surface>
      <View role="listitem" accessibilityLabel={`Personal details, ${data?.member.email ?? data?.member.phone ?? 'Available after sign-in'}`} style={styles.factRow}><Body>Personal details</Body><Body muted>{data?.member.email ?? data?.member.phone ?? 'Available after sign-in'}</Body></View>
      <View role="listitem" accessibilityLabel={`Membership, ${data?.membership ? `${data.membership.planName}, ${data.membership.status}` : 'No membership is visible'}`} style={[styles.factRow, styles.dividedRow, { borderColor: palette.decorativeSeparator }]}><Body>Membership</Body><Body muted>{data?.membership ? `${data.membership.planName} · ${data.membership.status.replaceAll('_', ' ').toLocaleLowerCase()}` : 'No membership is visible'}</Body></View>
      <View role="listitem" accessibilityLabel={`Gym, ${data ? `${data.gym.name}, ${data.gym.code}` : 'Available after sign-in'}`} style={[styles.factRow, styles.dividedRow, { borderColor: palette.decorativeSeparator }]}><Body>Gym</Body><Body muted>{data ? `${data.gym.name} · ${data.gym.code}` : 'Available after sign-in'}</Body></View>
      <View role="listitem" accessibilityLabel={`Appearance, ${modeLabel(appearance)}`} style={[styles.dividedRow, { borderColor: palette.decorativeSeparator }]}><Pressable accessibilityRole="button" accessibilityLabel={`Appearance, ${modeLabel(appearance)}`} onPress={() => setSettingsOpen(true)} style={styles.settingsRow}><View style={styles.settingCopy}><Body>Appearance</Body><Body muted>{modeLabel(appearance)} theme</Body></View><ChevronRight color={palette.secondaryText} size={UI_TOKENS.icons.navigationSize} /></Pressable></View>
    </Surface></View>
...
const styles = StyleSheet.create({
  ...
  profileCopy: { flex: 1, minWidth: 0, gap: UI_TOKENS.geometry.spacing[1] },
