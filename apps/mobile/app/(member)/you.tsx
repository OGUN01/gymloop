...
export default function YouScreen() {
  ...
  return <Screen>
    ...
    <View role="list" accessibilityLabel="Account" style={[styles.list, { borderColor: palette.decorativeSeparator }]}>
      <View role="listitem" accessibilityLabel={`Personal details, ${data?.member.email ?? data?.member.phone ?? 'Available after sign-in'}`} style={rowStyle}><Text style={rowText}>Personal details</Text><Text numberOfLines={1} style={rowValue}>{data?.member.phone ? formatPhone(data.member.phone) : data?.member.email ?? 'Available after sign-in'}</Text></View>
