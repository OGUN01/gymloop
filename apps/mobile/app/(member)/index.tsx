...
export default function MemberHome() {
  ...
  useEffect(() => { if (identity.kind !== 'member') return; const scope = { tenantId: identity.tenantId, userId: identity.userId }; void loadOfflineCheckIns(scope).then((rows) => setQueued(rows.length)); void drainOfflineCheckIns(scope, api).then((rows) => { if (rows.some((row) => row.result?.ok)) { setStatus({ text: 'A saved check-in is now confirmed.', tone: 'neutral' }); void reload(); } return loadOfflineCheckIns(scope); }).then((rows) => setQueued(rows.length)); }, [api, identity, reload]);
  if (loading) return <Screen><LoadingState /></Screen>;
  if (error || !data) return <Screen><Title>Home</Title><StateMessage tone="error">{error ?? 'Your gym information is unavailable.'}</StateMessage><ActionButton secondary onPress={() => void reload()}>Try again</ActionButton></Screen>;
  ...
  return <Screen>
    ...
    <Surface><Eyebrow>MEMBERSHIP</Eyebrow><Body>{data.membership ? `${data.membership.status} · ${data.membership.planName}` : 'No membership is visible'}</Body>{data.membership?.endsOn && <Body muted>Renews or ends {data.membership.endsOn}</Body>}</Surface>
