...
export default function ActivityScreen() {
  ...
  return <Screen><Eyebrow>YOUR PROGRESS</Eyebrow><Title>Activity</Title><Surface><Eyebrow>LAST 7 DAYS</Eyebrow><View style={styles.rhythm}>{rhythmDays.map((day) => <View accessible accessibilityLabel={`${day.fullLabel}: ${day.visited ? 'visited' : 'rest day'}`} key={day.fullLabel} style={styles.rhythmDay}><View style={[styles.rhythmDot, { backgroundColor: day.visited ? palette.primaryAction : palette.elevatedSurface, borderColor: day.visited ? palette.primaryAction : palette.requiredControlOutline }]} /><Eyebrow>{day.label}</Eyebrow></View>)}</View></Surface><Surface><Title>{data.streak.current}</Title><Body muted>{data.streak.unit === 'week' ? 'week' : 'day'} streak</Body>{data.streak.missed[0] && <Body>Keep going at your pace. A missed {data.streak.unit} does not erase the work you have done.</Body>}</Surface><Eyebrow>RECENT VISITS</Eyebrow>{data.visits.length === 0 ? <StateMessage>No confirmed visits yet.</StateMessage> : data.visits.map((visit) => <Surface key={visit.id}><Body>{new Date(visit.checkedInAt).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</Body><Body muted>{new Date(visit.checkedInAt).toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' })} · {visit.source === 'qr' ? 'Gym QR' : 'Desk assisted'}</Body></Surface>)}</Screen>;
}
