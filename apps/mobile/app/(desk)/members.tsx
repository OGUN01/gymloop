...
import { Body, Eyebrow, FONT, Initials, SearchField, LoadingState, Row, Screen, Status, Title, statusTone, statusWord, EmptyState, ErrorRetry } from '../../components/ui';
...
export default function MembersScreen() {
  ...
  const context = state !== 'ready' ? 'Edit members on the web console' : `${query.trim() === '' ? `${count} ${rows.length === 1 ? 'member' : 'members'}` : `${count} ${rows.length === 1 ? 'match' : 'matches'}`} · edit on the web console`;
  ...
  return <Screen>
    ...
    <View style={styles.roster}>
      ...
      {/* The one roster row (as on Check-in): name, phone, then the dot-and-word status on its own line; the member code
          is the right-hand reference column. Still rows: no chevron, no pressed state. */}
      {state === 'ready' ? <View>{rows.map((member) => <Row key={member.id} icon={<Initials name={member.fullName} />} title={member.fullName}
        meta={formatPhone(member.phone)}
        status={<Status tone={statusTone(member.status)}>{statusWord(member.status)}</Status>} statusBelow
        trailing={member.memberCode ? <Text numberOfLines={1} style={[styles.code, { color: palette.secondaryText }]}>{member.memberCode}</Text> : undefined}
...
const styles = StyleSheet.create({
  ...
  code: { fontFamily: FONT.medium, fontSize: UI_TOKENS.typography.compact.size, lineHeight: UI_TOKENS.typography.compact.lineHeight, fontVariant: ['tabular-nums'] },
});
