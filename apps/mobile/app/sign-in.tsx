...
export default function SignIn() {
  ...
  return <Screen><Image source={require('../assets/auth-gym-arrival-v1.png')} style={styles.hero} accessibilityLabel="A calm gym arrival" /><Eyebrow>GYMLOOP</Eyebrow><Title>Welcome back.</Title><Body muted>Sign in with the account your gym linked to you.</Body><ActionButton disabled={pending} onPress={() => void google()}>Continue with Google</ActionButton><View style={styles.divider}><View style={[styles.line, { backgroundColor: palette.decorativeSeparator }]} /><Body muted>or</Body><View style={[styles.line, { backgroundColor: palette.decorativeSeparator }]} /></View><ActionButton secondary disabled={pending} onPress={() => setEmailOpen((open) => !open)}>{emailOpen ? 'Hide email sign-in' : 'Use email instead'}</ActionButton>{emailOpen && <><Field accessibilityLabel="Email" autoCapitalize="none" keyboardType="email-address" placeholder="Email" value={email} onChangeText={setEmail} /><Field accessibilityLabel="Password" secureTextEntry placeholder="Password" value={password} onChangeText={setPassword} />{message && <StateMessage tone="error">{message}</StateMessage>}<ActionButton disabled={pending || email.trim() === '' || password === ''} onPress={() => void submit()}>{pending ? 'Signing in…' : 'Sign in'}</ActionButton></>}</Screen>;
}

const styles = StyleSheet.create({ hero: { width: '100%', aspectRatio: UI_TOKENS.geometry.media.authHeroAspectRatio, borderRadius: UI_TOKENS.geometry.radii.sheet, resizeMode: 'cover' }, divider: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: UI_TOKENS.geometry.spacing[2] }, line: { height: StyleSheet.hairlineWidth, flex: 1 } });
