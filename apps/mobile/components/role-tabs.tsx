...
export function RoleTabs({ desk = false }: { desk?: boolean }) {
  ...
  return <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: palette.primaryAction, tabBarInactiveTintColor: palette.secondaryText, tabBarStyle: { position: 'absolute', backgroundColor: palette.surface, borderColor: palette.decorativeSeparator, borderTopWidth: 1, height: UI_TOKENS.geometry.targets.touch + UI_TOKENS.geometry.spacing[4], left: UI_TOKENS.geometry.spacing[3], right: UI_TOKENS.geometry.spacing[3], bottom: UI_TOKENS.geometry.spacing[2] }, tabBarItemStyle: { minHeight: UI_TOKENS.geometry.targets.touch }, tabBarLabelStyle: { fontFamily: 'Inter_500Medium', fontSize: UI_TOKENS.typography.secondary.size } }}>{names.map(({ name, title, Icon }) => <Tabs.Screen key={name} name={name} options={{ title, tabBarIcon: icon(Icon) }} />)}</Tabs>;
}
