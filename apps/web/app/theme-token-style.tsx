...
export function ThemeTokenStyle() {
  const { geometry, icons, motion, opacity, typography } = UI_TOKENS;
  const tokenVariables = [
    ...
    ...Object.entries(icons).filter(([name]) => name !== 'strokeWidth').map(([name, value]) => `--gymloop-icon-${cssName(name)}:${value}px`),
    ...Object.entries(opacity).map(([name, value]) => `--gymloop-opacity-${cssName(name)}:${value}`),
