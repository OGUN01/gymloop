...
describe('owner dashboard metrics', () => {
  it.each([
    ...
    { label: 'authorized preview', identity: { kind: 'impersonation', userId: USER_ID, tenantId: response.tenantId, impersonationSessionId: STAFF_ID } },
  ...
  it('defaults absent dates and refuses unauthorized identities before RPC', async () => { const { loadOwnerMetrics } = await import('../../lib/metrics'); await loadOwnerMetrics(Promise.resolve({})); expect(state.rpc).toEqual([{ name: 'owner_metrics', args: { p_from: null, p_through: null } }]); state.rpc = []; state.identity = { kind: 'staff', role: 'trainer', userId: USER_ID, tenantId: response.tenantId, staffId: STAFF_ID }; await expect(loadOwnerMetrics(Promise.resolve({}))).rejects.toThrow(/^REDIRECT:/); expect(state.rpc).toHaveLength(0); });
