...
export function MemberNavigation() {
  ...
  const icons = [House, ChartNoAxesColumn, Dumbbell, UserRound] as const;
  return <nav className="member-tab-bar" aria-label="Member navigation">
    {MEMBER_DESTINATIONS.map((destination, index) => { const Icon = icons[index] ?? House; return <Link key={destination.href} href={destination.href} aria-current={destination.matches(pathname) ? 'page' : undefined}><Icon aria-hidden="true" size={UI_TOKENS.icons.navigationSize} strokeWidth={UI_TOKENS.icons.strokeWidth} /><span>{destination.label}</span></Link>; })}
  </nav>;
}
