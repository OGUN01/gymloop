...
/** A calendar date ("2026-10-12") as "12 Oct", keeping the year only when it is not the current one. */
export function memberShortDate(isoDate: string): string {
  return isoDate.startsWith(String(new Date().getFullYear())) ? formatDay(isoDate).replace(/ \d{4}$/, '') : formatDay(isoDate);
}


/** The attendance row for the same week the "N of goal" count uses; falls back to the last seven days. */
export function MemberWeekRhythm({ visits, timezone, weekStart }: { visits: readonly Visit[]; timezone: string; weekStart?: string | undefined }): ReactNode {
