...
export default async function MemberYouPage() {
  ...
  return <main className="member-route member-portal"><header><span>Account</span><h1>You</h1></header><YouSettings profile={memberProfile} /><section className="member-account-list" aria-label="Account details"><div><strong>Personal details</strong><small>{portal.member.email ?? portal.member.phone ?? 'Available after sign-in'}</small></div><div><strong>Membership</strong><small>{portal.membership ? `${portal.membership.planName} · ${portal.membership.status}` : 'No membership is visible'}</small></div><div><strong>Gym</strong><small>{portal.gym.branchName} · {portal.gym.gym_code}</small></div></section><form action={signOut}><button className="member-sign-out">Sign out</button></form><span className="sr-only">Settings are dismissible with Close or Back.</span></main>;
}
