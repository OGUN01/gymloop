...
type Profile = { full_name: string; email: string | null; phone: string | null; member_code: string | null };

export default function YouSettings({ profile }: { profile: Profile }) {
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [appearanceOpen, setAppearanceOpen] = useState(false);
  return <>
    <section className="member-profile"><div className="member-avatar" aria-hidden="true">{profile.full_name.slice(0, 1)}</div><div><strong>{profile.full_name}</strong><p>{profile.email ?? profile.phone ?? 'Member account'}</p><small>{profile.member_code ?? 'Verified member account'}</small></div></section>
    <button type="button" className="member-settings-trigger" aria-label="Open settings" onClick={() => setSettingsOpen(true)}><Settings aria-hidden="true" size={UI_TOKENS.icons.controlSize} /><span>Settings</span><ChevronRight aria-hidden="true" size={UI_TOKENS.icons.controlSize} /></button>
