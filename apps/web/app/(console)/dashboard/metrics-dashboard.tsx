...
/** A ratio as people say it — "2 of 9" — and its whole percent, rounded half-up; null when there is no cohort. */
function ratioParts(numerator: string, denominator: string): { value: string; percent: string | null } {
  const value = `${numerator} of ${denominator}`;
  const basisPoints = ratioBasisPoints(numerator, denominator);
  if (basisPoints === null) return { value, percent: null };
  const scaled = BigInt(basisPoints);
  const rest = scaled % BASIS_POINTS_PER_PERCENT;
  const whole = scaled / BASIS_POINTS_PER_PERCENT + (rest + rest >= BASIS_POINTS_PER_PERCENT ? BigInt(1) : BigInt(0));
  return { value, percent: `${whole}%` };
}
/** The range without its year when all of it falls in the current year ("1–24 Sep"); the period control keeps the full range. */
function shortRange(from: string, through: string, today: string): string {
  const range = formatDayRange(from, through);
  const [year = ''] = today.split('-');
  return from.startsWith(year) && through.startsWith(year) ? range.replace(` ${year}`, '') : range;
}
/** An instant in the gym's timezone as its day and its time ("19 Sep 2026", "9:00 am"). */
function splitInstant(value: string, timezone: string): { day: string; time: string } {
  const [day = '', time = ''] = formatSnapshotInstant(value, timezone).split(', ');
  return { day, time };
}
...
export function MetricsDashboard({ metrics }: { metrics: OwnerMetrics }) {
  ...
  const asOf = splitInstant(metrics.asOf, metrics.timezone);
  const primaryCards: Array<{ key: CardKey; label: string; value: string; scope: string; count?: string | undefined }> = [
    { key: 'visits', label: 'Visits today', value: metrics.cards.visitsToday, scope: `as of ${asOf.time}` },
    ...
    // The count ties the figure to the "N due" panel below; the range is the one the period control chose.
    { key: 'renewals', label: 'Renewals due', value: metrics.cards.renewal.length === 0 ? 'No renewals due' : metrics.cards.renewal.map((row) => formatMoney(row.currency, row.duePaise, true)).join(' · '), scope: range, count: renewalsDue === 0 ? undefined : `${renewalsDue} ${renewalsDue === 1 ? 'membership' : 'memberships'}` },
    { key: 'cash', label: 'Net collected', value: metrics.cards.cash.length === 0 ? 'No cash movement' : metrics.cards.cash.map((row) => formatMoney(row.currency, row.netPaise, true)).join(' · '), scope: range },
  ];
  ...
  return <main className="dashboard-workspace">
    ...
    <section className="dashboard-primary-metrics" aria-label="Primary metrics">
      {primaryCards.map((card) => <button aria-pressed={selected === card.key} className="dashboard-primary-metric" data-currency-context={card.value.includes('₹') ? `INR ${card.value}` : undefined} key={card.key} onClick={() => setSelected(selected === card.key ? null : card.key)} type="button"><span>{card.label}</span><strong>{card.value}</strong><small>{card.count === undefined ? null : <><span className="money-dash-count">{card.count}</span><span className="money-dash-count-sep"> · </span></>}<span className="money-dash-scope">{card.scope}</span></small></button>)}
    </section>
    ...
    <div className="dashboard-main-grid">
      <section className="dashboard-panel dashboard-cases" aria-labelledby="dashboard-cases-heading">
        {/* Title and its link share the title's baseline; the subtitle has its own line (money.css). */}
    ...
    {warningCount === 0 ? null : <section className="dashboard-quality" aria-labelledby="data-quality-heading"><h2 id="data-quality-heading">Data quality</h2>
      {metrics.warnings.undatedPayments.map((row) => <p key={row.paymentId} id={`warning-payment-${row.paymentId}`}><a href={`#warning-payment-${row.paymentId}`}>All-date data quality: undated payment {formatDisplayMoney(row.amountPaise, row.currency)}</a></p>)}
      {metrics.warnings.undatedReturns.map((row) => <p key={row.refundId} id={`warning-return-${row.refundId}`}><a href={`#warning-return-${row.refundId}`}>All-date data quality: undated return {formatDisplayMoney(row.amountPaise, row.currency)}</a></p>)}
