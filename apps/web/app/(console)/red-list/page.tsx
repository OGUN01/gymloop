...
export default async function RedListPage({
  ...
  return (
    <main className="follow-up-workspace">
            ...
            {cases.map((row) => (
              <li key={row.id} className="follow-up-row follow-up-case">
                <div className="follow-up-member follow-up-identity">
                  <span className="follow-up-initial" aria-hidden="true">{row.member_name?.charAt(0) ?? ''}</span>
                ...
                <div className="follow-up-attendance follow-up-absence">
                  ...
                  <strong>{row.days_absent} days away</strong>
                  <span>{row.last_attended_on === null ? 'never visited' : `last in ${row.last_attended_on}`}</span>
                </div>
                <p className="follow-up-history">
                  {row.last_follow_up_at === null ? 'Nobody has contacted them yet.' : `${row.last_follow_up_by ?? 'Someone'} tried ${row.last_follow_up_channel} — ${row.last_follow_up_outcome}`}
                </p>
                <MutationForm method="post" action="/api/follow-ups" className="follow-up-form">
                  ...
                  <label className="follow-up-field"><span>Channel</span><select name="channel" required className="follow-up-control">
                    {Constants.public.Enums.contact_channel.map((channel) => <option key={channel} value={channel}>{channel}</option>)}
                  </select></label>
                  <label className="follow-up-field"><span>Outcome</span><select name="outcome" required className="follow-up-control">
                    {Constants.public.Enums.follow_up_outcome.map((outcome) => <option key={outcome} value={outcome}>{outcome}</option>)}
                  </select></label>
      ...
      {nextHref === null ? null : <Link href={nextHref} rel="next" className="follow-up-next-page">Next page</Link>}
    </main>
  );
}
