...
const expectedScanCommand = 'select public.run_no_show_scan_all()';
...
function readLatestScheduledRun() {
  ...
  const query = "select j.command as job_command, d.command as run_command, d.runid::text as run_id, d.start_time::text as started_at, d.end_time::text as ended_at from cron.job j join cron.job_run_details d on d.jobid = j.jobid where j.jobname = 'no-show-scan-nightly' and j.active = true and j.schedule = '0 1 * * *' and d.status = 'succeeded' order by d.end_time desc limit 1";
  const output = execFileSync(process.execPath, [supabaseCli, 'db', 'query', '--linked', '--output-format', 'json', query], { encoding: 'utf8' });
  const rows = (JSON.parse(output) as { rows: Array<{ job_command: string; run_command: string; run_id: string; started_at: string; ended_at: string }> }).rows;
  expect(rows).toHaveLength(1);
  expect(rows[0]).toMatchObject({ job_command: expectedScanCommand, run_command: expectedScanCommand, run_id: expect.any(String), started_at: expect.any(String), ended_at: expect.any(String) });
  return rows[0];
}
...
test.describe('PILOT-009 manual scheduled no-show observer', () => {
  test('observes only the due scheduled scan through the staged owner identity', async ({ browser }) => {
    ...
    const firstObservation = today === secondObservationDay
      ? JSON.parse(readFileSync(observationPath(firstObservationDay), 'utf8')) as {
        runMarker: string; observationDay: string;
        cron: { runId: string }; noShowCase: { id: string; createdAt: string; count: number };
      }
      : null;
    if (firstObservation) {
      expect(firstObservation).toMatchObject({
        runMarker, observationDay: firstObservationDay, noShowCase: { count: 1 },
      });
    }
    ...
    try {
      ...
      if (firstObservation) expect(cron.run_id).not.toBe(firstObservation.cron.runId);
      ...
      const cases = await qaRead.from('no_show_cases')
        .select('id,tenant_id,member_id,status,opened_on,created_at,absent_days_at_open,threshold_days,last_attended_on', { count: 'exact' })
      ...
      expect(cases.data?.[0]).toMatchObject({
        ...
        absent_days_at_open: expectedNoShowThreshold + 1,
        threshold_days: expectedNoShowThreshold, last_attended_on: null,
      });
      const caseCreatedAt = cases.data?.[0]?.created_at ?? '';
      expect(Number.isFinite(Date.parse(caseCreatedAt))).toBe(true);
      if (firstObservation) {
        expect(cases.data?.[0]?.id).toBe(firstObservation.noShowCase.id);
        expect(caseCreatedAt).toBe(firstObservation.noShowCase.createdAt);
      } else {
        expect(Date.parse(caseCreatedAt)).toBeGreaterThanOrEqual(Date.parse(cron.started_at));
        expect(Date.parse(caseCreatedAt)).toBeLessThanOrEqual(Date.parse(cron.ended_at));
      }
      ...
      const observation = {
        ...
        cron: { jobCommand: cron.job_command, runCommand: cron.run_command, runId: cron.run_id, startedAt: cron.started_at, endedAt: cron.ended_at },
        stagedOwner: { userId: qaOwnerUserId, staffId: qaOwnerStaffId },
        ...
        noShowCase: { id: cases.data?.[0]?.id, status: cases.data?.[0]?.status, openedOn: cases.data?.[0]?.opened_on, createdAt: caseCreatedAt, absentDaysAtOpen: cases.data?.[0]?.absent_days_at_open, thresholdDays: cases.data?.[0]?.threshold_days, lastAttendedOn: cases.data?.[0]?.last_attended_on, count: cases.count },
      };
