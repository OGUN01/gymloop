...
### Add-on sales, fulfilment and returns

Owners, managers and front-desk staff may accept add-on sales; only owners and managers may publish offers or confirm a manual return. Trainers retain the Phase 2 staff-wide read policy for add-on orders and PT sessions, while every trainer mutation and fulfilment effect is restricted to orders assigned to that trainer's staff identity. Members may read active offers and their own frozen orders, but have no direct refund access. The member return RPC independently proves the tenant, member, order and payment relationship before exposing completed amount, currency, kind and processing time. The trainer-name RPC exposes only active PT offer ids and display names, with no staff UUID, contact or authentication field.

Sale acceptance locks the retry identity, catalogue, member and assigned trainer before checking the current quote and eligibility. The database fixes acceptance to `transaction_timestamp()`, derives the inclusive gym-local dates, requires zero initial PT usage and permits quantity one for PT and diet services. Positive-price sales create one ordinary paid manual payment whose bigint amount, currency, member and seller exactly match the frozen order; every attached payment remains an arrived, same-tenant, same-member, same-currency, exact-total non-membership payment on every later write. Complimentary orders can never attach a payment. Stock, payment, initial PT reservation, receipt and audit effects share the sale transaction, so a refusal leaves no partial state. Accepted facts and legal terminal states are database-enforced even for direct table writes.

Each elevated add-on lock/effect trigger treats the native `authenticated` database role, an authenticated JWT role, or a non-null JWT subject as an authenticated context. It then requires a complete claim and validates it against an active staff row with the same tenant, staff id, role and Auth subject before any privileged read or side effect. Stock and cancellation effects require front office, refund effects require an owner or manager, and PT consumption requires the assigned trainer. Trusted postgres/service paths retain their narrow provider or database behavior. Missing, contradictory or malformed authenticated identity claims are normalized to `42501` at these boundaries.

Every non-failed add-on refund must use the linked payment currency and shares the payment-row lock used by the aggregate refund ceiling. Requested and processing rows reserve that ceiling but do not count as returned cash. A newly completed full return moves only an eligible paid or active order to refunded and cancels its scheduled PT sessions; it neither restocks a delivered product nor reduces completed PT usage. Fulfilment guards use an exact, claim-scoped completed-return check so RLS-hidden refund rows cannot be mistaken for zero returned money.
...
| `razorpay_mandates` | **unresolved member-specific terminal event** | **8-year provisional engineering default; clock and basis need legal review** | `hold` pending field map | This reserved, currently disabled provider table has `member_id`, provider customer/subscription identifiers and `raw` JSON. It is member-linked personal/financial-adjacent data, **not** gym-only configuration. Do not delete or claim complete erasure until its export/redaction, hold scope and clock are approved. |
...
**`SUPABASE_DB_PASSWORD` was rotated on 2026-09-22; final verification remains.** The earlier temporary values were disclosed (ADR-051/145). Under ADR-148, a new random value was set for the exact Gymloop project, stored only in the gitignored local environment and GitHub Actions secret without printing it, and independently authenticated over CA-verified TLS. CI then applied the pending migration. Before customer launch, verify rejection of the old disclosed value through a password-using path and rerun the green CI credential path. Do not copy either value into source, evidence or chat.
