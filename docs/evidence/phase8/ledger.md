...
| HARD-003 | **Partial** | Earlier role/consent fixes through `e7a5883`; role/contrast/test commits `37f9c16`/`3bb641d`/`73def13`; member, desk and owner route tests `deb3e74`/`dc3e8b1`/`b24cfe3`; CI `35597428131` and `35597828557`; production deployment `dpl_DeQTPPHhisD8erdRe4GGg6YFCmEU`, 2026-09-21 | Local and production alias focused browser suite passed 4/4 for five role landings, forbidden routes, axe Light/Dark and English/responsive checks. The cold-runner 30-second combined-role timeout was fixed by splitting per role; CI/holdout/immutability then passed. Six member secondary routes passed 12/12 locally and CI/holdout/immutability passed. Four front-desk routes passed 8/8 locally; four owner routes passed 8/8 locally after distinguishing template delivery locale from product language controls (ADR-132). CI for desk/owner is pending. Earlier synthetic UI journeys and two distinct gym-owner sessions proved reciprocal foreign-member 404s, a foreign payment 404, one cross-gym check-in refusal with zero side effects, and tenant-scoped positive visit counts; exact IDs below. | Full Playwright-automated mutation-complete A–D journeys, reverse-direction direct mutation denial and complete two-gym browser acceptance remain open. The QA owner used Google OAuth in the in-app browser while Iron Box used a separate Playwright context, not two independently signed-in owners in the same checked-in run. Controlled second-gym credentials and disposable fixtures are needed for repeatable CI mutations. Owner: web acceptance/infrastructure owner. |
...
2/2. No application data mutation occurs in these scans. CI for the desk/
owner test increments must still be reconciled.
