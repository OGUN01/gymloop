...
financial, health, sign-in details, target audience (13+, owner decision) and
IARC content rating (Everyone / PEGI 3) are saved, not yet sent for review.
The five demo sign-ins were rotated through the Auth Admin API (new value works
for all five, old value rejected for all five, GitHub secret updated without
printing). Android now shows a no-access screen for a signed-in account with no
verified identity (HARD-011's Android half) and links the new public
`/privacy` and `/delete-account` pages. Production EAS build
`d94d9908-33a5-47a1-980e-f0f7ad6841f5` (versionCode 7, source `09318cf`) was
queued. No AAB upload, internal-track install or physical exact-artifact
checklist has happened yet, so HARD-008 stays Partial.
...
as unavailable on Free. The provider restore/PITR drill under gate 29 is still **unperformed**,
not Passed. A protected logical export succeeded on 2026-09-23 and its first
scheduled workflow run succeeded on 2026-09-24, as recorded below; no other
gate was waived at the September 22 checkpoint. The later ADR-168 privacy/legal and
missing-run pilot exceptions are recorded below; none changes a HARD gate to
Passed.
The full five-gym verdict remains **NO-GO** pending those other results.

**2026-09-23 owner pilot-scope decision (ADR-168):** the owner accepted
unresolved privacy product decisions and qualified legal/DPA review as
controlled-pilot exceptions until the planned VPS migration, and named themself
the primary GitHub alert responder. The owner also allowed the independent
missing-run alert to remain unverified for this pilot, although implementation
is being completed now. This is not a legal approval, privacy export/erasure
result, notification receipt or human acknowledgement. HARD-005/006 and the
related gates retain their actual statuses. The encrypted off-project R2
export remains active; provider restore/PITR remains unperformed. Other pilot
acceptance evidence, including A–D, exact Play artifact install and owner
visual acceptance, is still required for a truthful GO.
...
an observation, not an active restore plan. The protected export later
succeeded, but no Cloud import or provider PITR restore has run, so HARD-007
and gate 29 remain open.
...
Updated 2026-09-24. Status meanings: **Passed** has fetchable evidence for the
...
| HARD-005 | **Partial / watchdog and delivery routes verified** | Logger/API work through `bdfc81d`; evaluator `167c806`; provider-row repair `d673590`; failure escalation `3c689ac`; watchdog `b4bb7ac`; Cloudflare Worker version `d13d6872`, 2026-09-23 | Repaired production monitor evaluations passed from 12:30 onward. Deliberate failed run `35864351097` delivered closed TEST-only issue `#6`; closed TEST issue `#5` proves threshold alert delivery. The watchdog's forced TEST run `35873628703` delivered closed issue `#7` without `production-alert`; normal run `35874173699` passed. A real dual Worker dispatch at 14:30:39 UTC created green monitor `35874697518` and watchdog `35874696921`. Full script suite passed 576/576, CI/holdout/immutability green for `b4bb7ac`. | Observed cadence is finite, not a guarantee, and no naturally missed production run has occurred. The named owner has not acknowledged a GitHub TEST alert; protected role roster and retention/access review remain. The owner excepted an unverified independent missing-run alert for the controlled pilot, but this implementation now has a TEST issue receipt. |
...
| HARD-007 | **Partial / verified logical backup; external restore** | Runbooks `055e83c`; manual export `35846608557`, 2026-09-23; first scheduled run `35959494549`, 2026-09-24 | Manual encrypted public/Auth Cloud export was stored in private R2, read back, decrypted and hash-verified; redacted receipt below. First scheduled `export` job succeeded and GitHub lists its receipt artifact, whose contents were not inspected. No tabletop or provider restore/PITR drill was performed. | Owner: production owner/platform administrator; Privacy Lead for legal notification confirmation. Verify later daily receipts and cadence, role/contact roster, tabletop and legal/DPA decision. Gate 29 remains excepted for controlled pilot, external/unperformed for general release. |
...
The fourth run, marker `PHASE8-LOAD-5bd1006e-20d0-49da-bb90-e82339ae1eb0`,
again completed all 50,000 iterations with zero interrupted and both
cross-tenant denials. Its check-in p95 was **1,760.5 ms**, within the unchanged
2,000 ms budget, but only **49,971/50,000** check-ins returned HTTP 200; the
other 29 returned HTTP 500 during the opening burst. The error codes were not
captured, so their cause is unproven. The maximum observed database size was
126,905,491 bytes, below the 400,000,000-byte abort ceiling. Raw k6 JSONL
SHA-256:
`2E34BCB8AE6E1279210278FF3776C3EDDB59D35C4AD31E6DCF463B3BD2A4CA0`.
The recovery manifest reports completed cleanup, unchanged pre-existing rows,
zero synthetic database rows and zero synthetic Auth users. A parser defect
that rejected unrelated global k6 telemetry was separately fixed after the
run; reparsing the immutable raw result produced the measurements above.
The bounded same-event retry for a PostgREST pool-acquisition timeout is
committed and independently reviewed, but this run predates it. HARD-004
remains **NO-GO** pending an exact rerun with 50,000 acknowledgements.

Attempt `PHASE8-LOAD-cc2890c7-f28f-4810-982e-c5077d4a1e09` stopped
**before k6** when Cloud Auth sign-in for the 33rd synthetic gym returned
`fetch failed`. Its manifest reports complete cleanup, unchanged pre-existing
rows and zero database/Auth remnants; an independent linked preflight confirmed
the same at 99,134,611 database bytes. It contributes no latency result.

The fifth complete run, marker
`PHASE8-LOAD-0e24cb17-5843-4055-b6bf-bf97146e138a`, finished 50,000
iterations with zero interrupted and both cross-tenant denials. Its check-in
p95 was **1.24 s**, below the fixed 2 s budget, but only **49,987/50,000**
returned HTTP 200; 13 returned HTTP 500. All 13 failures ended within the
opening burst at 11:18:28 UTC after about 10.8–10.9 s. The underlying
PostgREST error code was not captured, so the cause and the bounded retry's
effect remain unproven. The maximum observed database size was 128,052,371
bytes, below the 400,000,000-byte abort ceiling. Raw k6 JSONL SHA-256:
`1EFE4D49C3CAB7F9147EA867C2754C8501A6ADB76405808770F370DDFC664949`.
The recovery manifest reports complete cleanup, unchanged pre-existing rows,
zero synthetic database rows and zero synthetic Auth users; an independent
preflight reconfirmed zero remnants at 100,035,731 database bytes. A separate
red test and reviewed diagnostic now log only the verified tenant and a
validated database code on future generic check-in errors. This fifth run
predates that diagnostic. HARD-004 remains **NO-GO**.

A **diagnostic only** 100-gym × one-check-in opening burst, marker
`PHASE8-LOAD-39f1abeb-c8f4-4d42-91e4-eda79259b17a`, reproduced 31 HTTP
500s and 69 HTTP 200s from 100 iterations; it is not a substitute for the
frozen 100 × 500 gate. The rebuilt route emitted exactly 31 safe
`check_in.database_error` events, all with `code: unclassified`; no database
message or request identifier was logged. Its raw k6 JSONL SHA-256 is
`8E8529C8437592C8771A02099DA14A72994394916346E8DC82A2DF266AE8E9F3`.
The diagnostic harness differed from the canonical k6 script only in its
one-iteration setting, and its runner differed only in that script path.
The installed `@supabase/postgrest-js` 2.112.4 source returns status `0` and
an empty error code when a POST fetch or response read rejects, and does not
automatically retry POST. That is a plausible explanation for these
unclassified failures, not direct proof of their uncaptured status or whether
SQL ran. The manifest and a separate
linked preflight confirmed complete cleanup, unchanged pre-existing data and
zero synthetic database/Auth remnants (100,084,883 database bytes). The
...
Its six linked Supabase CLI captures included separate `public` and `auth`
data-only exports; the fail-closed validator required complete, nonempty
top-level public and `auth.users` COPY blocks. The runner encrypted the archive
before uploading object
`2026-09-23/gymloop-cloud-35846608557-1.enc` to private R2 bucket
`gymloop-backups`, then downloaded the exact object, verified its ciphertext
SHA-256 `7519297e6d708d4d16622e92769d85c0871f085a2029b086fa0c944e029b1c61`,
decrypted it and verified the plaintext and all four source-part hashes.
The redacted receipt reports capture time `2026-09-23T10:13:41.262Z`,
5,505,826 encrypted bytes, and `verified: true`. Its GitHub artifact is
`protected-backup-receipt-35846608557-1` (30-day retention); the downloaded
receipt SHA-256 is
`0D01982527EF933728708B0968EBAC69352B7176A41B525FB736D43B5562E76A`.
No raw SQL, encryption key or credential was added to this ledger or repository.

The bounded backup parser critic returned GO after a separate red test commit,
37 focused visible tests passed, and the live run resolved two earlier failed
export attempts. This establishes one encrypted off-site logical export and
read-back receipt. The six captures were sequential, so application restore
consistency remains unproven. The first scheduled execution completed on
2026-09-24: [run `35959494549`](https://github.com/OGUN01/gymloop/actions/runs/35959494549)
(event `schedule`, source `8b8b1f7e169aa7d794c78fa951f3cea5d825c601`)
was created at 05:20:20 UTC; its `export` job ran from 05:20:23 to 05:29:43
UTC and concluded `success` (workflow completed by 05:29:44 UTC). GitHub lists
artifact `protected-backup-receipt-35959494549-1`. This check did not open
that artifact or read its contents: one scheduled successful run and artifact
listing do not establish its receipt hashes, ongoing cadence, application
consistency or restorability. The owner-excepted provider restore/PITR drill
remains unperformed and gate 29 remains open.
...
**2026-09-23 watchdog closeout:** independent visible and blind holdout tests
were committed before implementation `b4bb7ac`. The full script suite passed
576/576, lint and registry checks passed, and a fresh critic gave code-level
GO after the invalid-calendar-date parser was corrected. CI, holdout and test
immutability were green on the pushed commit. The previous long DB workflow
`35864337526` also completed successfully; the new DB workflow started and
had migration, rollback and schema-drift green while pgTAP was still running.
The production monitor's manually dispatched run `35873020959` completed with
the exact `Gymloop production monitor` title. A forced watchdog TEST run
[`35873628703`](https://github.com/OGUN01/gymloop/actions/runs/35873628703)
created [issue #7](https://github.com/OGUN01/gymloop/issues/7) with only TEST
labels and no `production-alert`; after verification it was closed. Normal
watchdog run [`35874173699`](https://github.com/OGUN01/gymloop/actions/runs/35874173699)
succeeded and there were no open production alert issues. The Cloudflare
dashboard showed Worker version `d13d6872` active after the dual-dispatch edit.
At 14:30:39 UTC it created separate workflow-dispatch runs
[`35874697518`](https://github.com/OGUN01/gymloop/actions/runs/35874697518)
(monitor) and [`35874696921`](https://github.com/OGUN01/gymloop/actions/runs/35874696921)
(watchdog); both completed green with their failure jobs skipped. The Cloudflare
Cron-events page had not yet refreshed its 14:30 row when checked, so the two
matching GitHub dispatch receipts and active Worker version are the recorded
...
| Monitoring destination and alert route | HARD-005 | Closed TEST issues #5 (threshold alert), #6 (collection failure) and #7 (missing-run watchdog) delivered; Cloudflare Cron version `d13d6872` dispatched both production workflows at 14:30:39 UTC and both passed | Production owner obtains actual named responder acknowledgement and protected roster/access review | Human acknowledgement and role/access review; no natural missed-run incident or absolute scheduler guarantee is claimed |
...
| Supabase cloud backup/PITR restore | HARD-007 | Manual encrypted logical export and exact read-back passed in `35846608557`; first scheduled `export` succeeded in `35959494549` and listed a receipt artifact (not opened here); provider restore/PITR remains excepted for controlled pilot and unperformed | Production owner verifies each later backup receipt; project administrator conducts the later approved provider drill | Inspect scheduled receipt integrity and continued daily cadence; later cloud operation id, source/distinct target, timings, validation and cleanup |
