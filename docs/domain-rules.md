...
- **ATT-009** WHEN a gym is created after this migration, THE SYSTEM SHALL default its check-in mode to `printed_poster`; WHEN an existing gym is migrated, THE SYSTEM SHALL preserve its `rotating_screen` behaviour until an authorized owner or manager changes it. One mode belongs to the gym, not a member-supplied or per-scan value.
