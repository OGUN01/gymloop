...
| `app.current_app_role()` | `supabase/migrations/20260907184313_phase2_identity_accessors_and_hook.sql` | Reads the `app_role` JWT claim **as `text`, casting nothing**; `null` when absent. An unrecognised role is compared as text like any other, so it grants nothing and raises nowhere — a cast to `public.app_role` would make one forged claim silent on the gate functions and `22P02` on the policies that compare a label directly | Both `<table>_platform_write` clauses (`= 'super_admin'`); the `= 'gym_owner'` write gates on `organizations`, `staff`, `razorpay_accounts`; both `platform_users` policies |
