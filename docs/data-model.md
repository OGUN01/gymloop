...
- `refund_kind`: `refund`, `reversal`. `refund_status`: `requested`, `processing`, `completed`, `failed` (requested→processing→completed|failed; requested→failed). **`completed` is terminal and enforced** (`GL041`, round seventeen): the ceiling sums refunds that are not `failed`, so demoting a completed refund freed the room it was consuming and a second full refund was accepted — money that left becoming an attempt that never happened. `processing→failed` stays permitted, because a refund in flight can genuinely fail; what bounds that case is the ceiling running on UPDATE, so at most one of two refunds ever reaches `completed`. `currency` and `kind` remain writable for the life of the row — OPEN-032, inert today because the ceiling reads neither.
