...
- Phase 6 direct-row invariants, enforced on RPC and direct writes by `app.enforce_lead_discipline()` (the recreated `leads_touch_updated_at` trigger): a lead is created only at `new` with null trial/conversion/loss fields; `trial_at` is required exactly on `trial_scheduled`/`trial_done`; `lost` requires a trimmed nonempty reason and every other stage forbids one; `converted` requires a same-tenant eligible member (status not `cancelled`/`blocked`, `erased_at` null) and server conversion time, while every other stage requires both conversion fields null; a terminal row's business facts cannot change; `id`, `tenant_id`, `created_at` and both evidence pairs are immutable.
