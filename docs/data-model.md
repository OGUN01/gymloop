...
- `periods_granted integer =0` — how many plan-length periods payments have granted this membership, `check (periods_granted >= 0)`. The constraint exists because a critic set it to `-11`, took one ordinary payment and moved `ends_on` a full year — a free year attributed in the ledger to a legitimate month's payment. **Recorded, not derived** (ADR-087): an `AFTER` trigger cannot see the money total as it stood before its own statement, so "how much is already granted" has to be a fact rather than a subtraction. Three rounds of Phase 5 derived it and over-granted in three different shapes — ten statements, ten rows in one statement, and an upsert firing both statement triggers.
