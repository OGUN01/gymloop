...
| migration file | `supabase migration new <cluster>` supplies the timestamp; if it would precede an existing version, rename that new CLI-created file to the next second after the highest existing version — never renumber history (ADR-112) | `20260906120000_tenancy.sql` |
...
- `updated_at timestamptz not null default now()` on exactly the tables the list gives one, each maintained by the shared trigger below and by nothing else. A table with no `updated_at` gets no `<table>_touch_updated_at` trigger. (That sentence used to read "gets no trigger", full stop, and Phase 3 made the shorter reading false: `attendance` has no `updated_at` and now carries `attendance_enforce_check_in`, a `before insert` trigger that is what makes exactly-once true — ADR-061. The rule is about the timestamp, not about triggers in general.)
...
| append-only | `select, insert` | `attendance_corrections`, `follow_ups`, `consents` |
| read-only | `select` | `messaging_wallets`, `messaging_wallet_ledger`, `webhook_events`, `audit_log` — **ADR-047**, extended by **ADR-049**. All four are records *about* the gym that the gym must not be able to write. A signed-in gym user holding `update` on `messaging_wallets` can set its own credit balance to any number, leaving no ledger row; a gym holding `insert` on `audit_log` can forge a row naming a platform user as the actor, and nobody can ever delete it. None has a v1 flow in which a user session writes it: the wallet's arithmetic, its ledger, every audit write and every webhook record run on `service_role`, which is never revoked from. |
...
**ADR-049 moved two more tables in.** Leaving `messaging_wallet_ledger` append-only defeated ADR-047's own wallet fix: the contract says the balance *is* the sum of the ledger, so a gym barred from updating `messaging_wallets` could simply insert its own credit rows. And a gym holding `insert` on `webhook_events` can forge the record of a payment webhook it verified itself, `signature_valid` included — the same argument ADR-047 made for `audit_log`. Neither has a v1 flow in which a user session writes it.

Read-only and append-only are both privileges, not triggers: an insert-only grant cannot be forgotten in a code path the way a guard can. `webhook_events` is **read-only** for `authenticated` (ADR-049): both the row and its `processed_at` stamp are written by the webhook Edge Function on `service_role`, and a gym that could insert one would be forging the record of a webhook it verified itself, `signature_valid` included.
...
2. Every foreign-key column is indexed: it either leads an index of its own, or it sits immediately after the tenant column in a tenant-leading composite — `(tenant_id, member_id, checked_in_at desc)` indexes `attendance.member_id`, and no separate one is added. The second form counts because every read here is tenant-scoped, and the referential-integrity scan a standalone index would serve happens only when a parent key is deleted or updated: Phase 1 grants `delete` nowhere, and a uuid key is never updated. Unlike rule 1, **rule 2 accepts a partial index** — `no_show_cases.member_id` sits immediately after `tenant_id` in the partial `(tenant_id, member_id) where status in (…)` (ADR-047 tenant-scoped it; it used to lead `(member_id) where status in (…)` alone), `members.user_id` sits only in `(tenant_id, user_id) where user_id is not null`, and `staff.user_id` sits only in the same shape, `(tenant_id, user_id) where user_id is not null` — all three are sufficient, because the RI probe is `where <fk> = $1`, which implies the partial predicate, so the planner can use the index. The meta-test must encode this difference or it will fail three tables that are correct.
3. Every column appearing in an RLS policy predicate is indexed. **This rule is about `using`, not `with check`** — a `using` clause selects rows and an unindexed column there means a scan; a `with check` is evaluated against one row already in hand and drives none. The meta-test reads `polqual` and not `polwithcheck`, deliberately.
4. **Every column a *per-request function* filters on is indexed too, and this rule was missing until it cost something.** Rule 3 governs policy predicates, and the meta-test measured every one of them — but the access-token hook queries `staff` and `members` by `user_id` alone, those are not policy predicates, and nothing was watching them. The only indexes on that column led with `tenant_id` and could not serve `where user_id = $1`, so the hook ran up to four sequential scans **on every token mint and every refresh**, against a two-second budget whose timeout is silently swallowed. A third blind critic found it; the implementer named the general form. **Anything that runs on every request carries a policy's index obligation**: the auth hook now, a Route Handler's query next. The rule is stated because fixing the instance would have left the next one to be discovered the same way. With the template above that is the tenant column, so rule 1 discharges it; it is stated separately because it stops being automatic the moment a policy grows a second term.
...
| attendance | `attendance_source` (contract), `checkin_gate_mode` (gate-mode migration) |
...
- `tenant_id uuid` pk → organizations · `preset gym_preset ∅` · `logo_url text ∅` · `brand_accent text ∅` check `^#[0-9a-fA-F]{6}$` · `address_line1 text ∅` · `address_line2 text ∅` · `city text ∅` · `state text ∅` · `pincode text ∅` check `^[1-9][0-9]{5}$` · `gstin text ∅` check `^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$` · `invoice_prefix text =INV` · `receipt_prefix text =RCPT` · `financial_year_start_month smallint =4` check 1–12 · `week_start_day smallint =1` check 0–6 (0 = Sunday) · `opening_hours jsonb ={}` · `no_show_threshold_days smallint =7` check > 0 · `checkin_dedupe_seconds integer =120` check ≥ 0 (ATT-004 window) · `checkin_gate_mode checkin_gate_mode =printed_poster` (ATT-009; existing rows backfilled to `rotating_screen`) · `streak_rule_type streak_rule_type =visit_streak` · `weekly_goal_default smallint =3` check 1–14 · `renewal_reminder_days_from_expiry smallint[] ∅` (null = platform default `RENEWAL_REMINDER_WINDOWS`; same axis: negative before, positive after) · `grace_period_days smallint =0` check ≥ 0 · `pause_reasons text[] ={}` · `pause_approver_role app_role =gym_manager` check in (`gym_owner`, `gym_manager`, `front_desk`, `trainer`) · `max_freeze_days_per_year smallint =30` check ≥ 0 · `trainer_member_cap smallint ∅` check > 0 · `created_at`, `updated_at`
...
- `member_id` — **frozen** (`GL042`, round seventeen). A membership belongs to the member it was sold to; `GL042`'s own rule is evaluated on the payment row, so this column was governed by nothing and a front-desk session moved a membership carrying a granted period to another member in one statement, the paid payment still naming the original. Correcting who a membership was sold to is a refund, a cancellation and a new sale.
- `starts_on` / `ends_on` — when the membership runs from and until. **Written by `app.grant_periods()` and by nothing else** (`GL045`, ADR-093): changeable only at `pg_trigger_depth() >= 2`, the same rule `periods_granted` has. Nine rounds governed the price, the length and the count while these — the output of all that arithmetic — were writable by hand: `set ends_on = starts_on + 3650` was allowed from an ordinary front-desk session, and `starts_on` is read by the check-in gate (ADR-084), so moving it is a day at the turnstile as well as a month on the ledger. Creation sets them and is deliberately unpoliced (OPEN-029).
- `duration_days integer not null` — how long one period of this membership is, **as sold**, `check (duration_days > 0)`, taken from the plan by `app.stamp_membership()` on insert **and on any update that changes the plan**, whatever the caller named; the default of `1` is never observed, because that trigger replaces it and an unreadable plan is refused by the composite foreign key. Written any other way it is **refused** with `GL043` (ADR-092), at any time and whatever the money — typing `3650` onto a 30-day membership and paying its ordinary price bought ten years, and left a record no audit could distinguish from an honest one. Recorded rather than read from `plans.duration_days` when money arrives (ADR-090): `plans_tenant_write` is `FOR ALL` on `is_gym_admin()`, and one manager statement setting `duration_days = 3650` followed by an ordinary renewal moved `ends_on` **3650 days**, on every membership sold on that plan at once. A membership already recorded the price, discount and currency it was sold at; the duration was the one term still read from somewhere else, and that asymmetry was the defect. Frozen with the other four terms once any money has arrived (`GL043`).
- `periods_granted integer =0` — how many plan-length periods payments have granted this membership, `check (periods_granted >= 0)`, and **written by the granting rule and by nothing else** (`GL044`, ADR-089) — `memberships_terms_frozen` refuses any change to it below `pg_trigger_depth() = 2`, which is where `app.grant_periods()`' own write runs. The CHECK came first, from a critic who set the column to `-11`, took one ordinary payment and moved `ends_on` a full year. It closed the sign and nothing else: the next critic set the column to `0` and bought a month for **one paisa**, and set it to `500` to make a real ₹1,000 payment grant nothing at all while still being receipted. Both used values the rule itself produces, so no constraint on the value could have caught them — **what was wrong was that a hand wrote it**. The same trigger freezes `price_paise`, `discount_paise`, `currency` and `plan_id` **once any money has arrived** — `paid`, `refunded` or `reversed`, in any currency (`GL043`, ADR-090). A period is scored against the nonnegative sold net price, `price_paise - discount_paise`; a zero net price displays zero fee and neither divides nor grants payment-driven periods. **This bullet said "once this count is above zero" for two rounds after ADR-090 replaced that gate**, which is the superseded rule stated as current in the document the routing table sends readers to for column write rules; the old gate is what left a part-paid membership wide open, measured at ten years for one paisa. The reason given was wrong too: `app.grant_periods()` reads the price, discount and currency off the membership and **does not read `plan_id` at all** — the join to `plans` was deleted in the same ADR. `plan_id` is frozen because it is a term of the sale, not because the rule re-reads it. And **who** may change any of them is `GL046` (ADR-094): a gym admin, never the front desk, at creation as well as afterwards — the newest rule on these columns and the only claim rule among them. **Recorded, not derived** (ADR-087): an `AFTER` trigger cannot see the money total as it stood before its own statement, so "how much is already granted" has to be a fact rather than a subtraction. Three rounds of Phase 5 derived it and over-granted in three different shapes — ten statements, ten rows in one statement, and an upsert firing both statement triggers.
...
- `id uuid` pk · `tenant_id` → organizations · `payment_id` → payments · `kind refund_kind` · `amount_paise bigint` check > 0 · `currency text =INR` · `status refund_status =requested` · `provider_refund_id text ∅` · `idempotency_key text ∅` · `reason text` check ≠ '' · `initiated_by_staff_id uuid ∅` → staff · `processed_at timestamptz ∅` · `created_at`, `updated_at`
- Indexes: `payment_id`; `initiated_by_staff_id`; unique `(tenant_id, provider_refund_id) where provider_refund_id is not null`; unique `(tenant_id, idempotency_key) where idempotency_key is not null`.
- Privileges: no `delete` for `authenticated`.
- `id`, `idempotency_key`, `payment_id`, `amount_paise`, `currency` and `kind` are immutable (`GL041`); historical null keys remain null. The invoker `public.record_refund` requires a real owner/manager staff claim and a UUID key. An exact same-gym replay returns the original refund without a write; changing payment, amount, currency, kind or current reason under that key answers `GL048`. The product trims reason whitespace before calling the RPC.
- Named project-owned refusal order is `GL041`, `GL040`, `GL036`; payment order is `GL038`, `GL042`, `GL039`, `GL034`, `GL035`. These orders do not rank PostgreSQL constraints, privileges or RLS. Accepted payment/refund INSERTs and UPDATEs append an atomic financial audit event; equivalent keyed retries append none. See `docs/security.md` and the fixed refund contract for summary and actor fields.
...
- Privileges: read-only for `authenticated` (ADR-049); no `delete` for anyone but `service_role`. No `updated_at`: rows are written once, then `processed_at` is stamped.

**`invoices`** — GST invoices, numbered per gym per financial year. Tenant path: direct.
- `id uuid` pk · `tenant_id` → organizations · `payment_id` → payments, unique **per gym** (`(tenant_id, payment_id)`, **ADR-049** — a global unique on this column is the fifth instance of the ADR-047 class: gym A can name gym B's payment id in its own invoice and take the slot gym B needs, permanently) · `invoice_number text` · `financial_year text` check `^[0-9]{4}-[0-9]{2}$` (e.g. `2026-27`) · `issued_at timestamptz =now()` · `seller_gstin text ∅` · `buyer_name text` · `buyer_gstin text ∅` · `place_of_supply text ∅` · `taxable_paise bigint` check ≥ 0 · `cgst_paise bigint =0` check ≥ 0 · `sgst_paise bigint =0` check ≥ 0 · `igst_paise bigint =0` check ≥ 0 · `total_paise bigint` check ≥ 0 · `currency text =INR` · `line_items jsonb =[]` · `pdf_url text ∅` · `created_at`, `updated_at`
- Indexes: unique `(tenant_id, invoice_number)`; `(tenant_id, financial_year)`.
- Privileges: no `delete` for `authenticated`.

**`document_counters`** — next number per gym, per document kind, per financial year (invoice prefix + financial-year reset computed in `app.stamp_payment()`, which **hardcodes 1 April**. Nothing in the money path reads `organization_settings.financial_year_start_month` or `receipt_prefix`; both are decoration today, and harmless only while every gym is Indian). Tenant path: direct.
...
**`qr_sessions`** — QR gate sessions for the gym's `rotating_screen` or `printed_poster` mode (ATT-003/009–014). Only the token hash is stored; printable poster code is HMAC-derived from a server-only secret and the random session ID and checked against that hash on redisplay. Tenant path: direct.
- `id uuid` pk · `tenant_id` → organizations · `branch_id` → branches · `token_hash text` unique · `gate_mode checkin_gate_mode =rotating_screen` (historical sessions keep their mode) · `issued_at timestamptz =now()` · `expires_at timestamptz ∅` (required and > issued_at for rotating; null for poster) · `revoked_at timestamptz ∅` · `created_by_staff_id uuid ∅` → staff · `created_at`
- Indexes: `(tenant_id, expires_at)`; `branch_id`; `created_by_staff_id`; unique partial `(tenant_id, branch_id) where gate_mode = 'printed_poster' and revoked_at is null` (one live poster per branch, including concurrent replacements).
- `public.replace_checkin_poster` atomically revokes the branch's old poster and inserts a hash-only replacement; `public.set_checkin_gate_mode` revokes all active posters when returning to rotating. Both derive tenant/actor from a verified owner/manager JWT and write an attributed audit entry. A direct authenticated write cannot bypass these commands. Poster attendance enforces tenant, member branch, live membership, opening hours and one attendance per branch-local calendar day at trusted arrival time in the shared BEFORE INSERT trigger; rotating expiry/offline semantics remain unchanged.

**`attendance`** — one row per visit. Check-out is optional (ATT-008). Tenant path: direct.
- Staff-assisted front-desk writes use `public.record_staff_front_desk_check_in(uuid,text,uuid)`: a volatile invoker command that finds the member through the caller's `members` SELECT policy, derives tenant and actor from the JWT, then inserts through the existing attendance policy and trigger. An absent or foreign member returns no row. The route resolves exact client-event retries by selecting the already visible attendance row.
...
- `id uuid` pk · `tenant_id` → organizations · `kind addon_kind` · `name text` · `description text ∅` · `price_paise bigint` check ≥ 0 · `currency text =INR` · `gst_rate_bp smallint =0` check 0–10000 · `validity_days integer ∅` check > 0 · `session_count integer ∅` check > 0 · `trainer_staff_id uuid ∅` → staff · `trainer_qualification text ∅` · `stock_quantity integer ∅` check ≥ 0 (DQA-004/ADD-004 made structural) · `cancellation_terms text ∅` (ADD-002) · `quote_version uuid` · `is_active boolean =true` · `sort_order smallint =0` · `created_at`, `updated_at`
...
- Active offers are sellable only when their common disclosure and kind-specific fields are complete. Changing a disclosed acceptance term rotates `quote_version`; stock-only and display-order edits preserve it. Once referenced by an order, `kind` is permanent. Incomplete legacy offers stay readable but cannot be newly sold.

**`addon_orders`** — a member's purchase of an add-on and its usage state. Tenant path: direct.
- `id uuid` pk · `tenant_id` → organizations · `member_id` → members · `addon_product_id` → addon_products · `payment_id uuid ∅` → payments · `status addon_order_status =pending` · `quantity integer =1` check > 0 · `unit_price_paise bigint` check ≥ 0 · `total_paise bigint` check ≥ 0 · `currency text =INR` · `trainer_staff_id uuid ∅` → staff · `sessions_total integer ∅` check > 0 · `sessions_used integer =0` check ≥ 0 · `starts_on date ∅` · `expires_on date ∅` check ≥ starts_on · `idempotency_key text ∅` · `sold_by_staff_id uuid ∅` → staff · `sale_snapshot jsonb ∅` · `sale_request jsonb ∅` · `initial_session_id uuid ∅` → pt_sessions · `sold_at timestamptz ∅` · `cancelled_at timestamptz ∅` · `created_at`, `updated_at`
...
- Indexes: unique `(tenant_id, idempotency_key) where idempotency_key is not null`; unique `(tenant_id, payment_id) where payment_id is not null`; `member_id`; `addon_product_id`; `payment_id`; `sold_by_staff_id`; `initial_session_id`; `trainer_staff_id`; `(tenant_id, status)`; `(tenant_id, sold_at)`.
- Privileges: no `delete` for `authenticated` (INT-001).
- `record_addon_sale` is the atomic acceptance path. It locks the request identity and offer, verifies the current quote, derives exact bigint money and the inclusive gym-local validity window, freezes the normalized request and disclosed snapshot, creates the exact manual payment and initial PT reservation when applicable, and applies stock once. Exact retries are read-only; changed facts under the key are `GL052`.
- Identity, seller, request, accepted money, disclosure, validity, payment link, trainer and usage facts are permanent. Acceptance uses the transaction timestamp and derives its gym-local dates; PT starts with zero usage, and PT/diet quantity is exactly one. Every attached payment is arrived and matches the exact tenant, member, total and currency without a membership; complimentary orders never attach one. Stored transitions are only pending→paid/cancelled, paid→active/cancelled/refunded and active→completed/refunded; completed, cancelled and refunded are terminal. Product sales complete inside acceptance, diet plans complete only while active and within the inclusive sold window, and PT completes only when every purchased session has been consumed.
...
- Constraints: `exclude using gist (tenant_id with =, trainer_staff_id with =, tstzrange(starts_at, ends_at) with &&) where (status in ('scheduled','completed'))` — the leading `tenant_id` term is **ADR-047**: without it a gym can fill another gym's trainer's calendar with rows that gym cannot see, read or delete — needs `btree_gist`, created in this cluster's migration. Unique `(tenant_id, id)` added with the Phase 6 add-on sale cluster.
- Indexes: `addon_order_id`; `member_id`; `(tenant_id, trainer_staff_id, starts_at)`; `(tenant_id, addon_order_id, status)`.
- New reservations require the assigned real trainer, an active order that has not been fully returned, a start at or after the current command, an exact slot inside the purchased gym-local window and remaining purchased capacity. Identity, slot and notes are permanent. Only scheduled→completed/cancelled/no_show is legal; completed consumes one session once and only after the slot ends while cancelled/no-show frees the reservation.
...
**`messaging_wallet_ledger`** — every credit movement; the balance is the sum. Append-only to `service_role`, **read-only to `authenticated`** (ADR-049 — the balance being the sum is exactly why a gym that can insert here can mint its own billing credits). Tenant path: direct.
...
- Privileges: read-only for `authenticated` (ADR-049); `service_role` inserts.
...
- `id uuid` pk · `tenant_id` → organizations · `branch_id` → branches · `full_name text` · `phone text` check E.164 · `email text ∅` · `source lead_source` · `stage lead_stage =new` · `assigned_to_staff_id uuid ∅` → staff · `trial_at timestamptz ∅` · `converted_member_id uuid ∅` → members · `converted_at timestamptz ∅` · `lost_reason text ∅` · `notes text ∅` · `created_by_staff_id uuid ∅` (Phase 6) · `creation_request_key uuid ∅` (Phase 6) · `creation_request_facts jsonb ∅` (Phase 6) · `conversion_request_key uuid ∅` (Phase 6) · `conversion_request_facts jsonb ∅` (Phase 6) · `revision uuid` (Phase 6) · `created_at`, `updated_at`
- Checks: `stage <> 'converted' or converted_member_id is not null`.
- Indexes: `(tenant_id, stage)`; `(tenant_id, phone)`; `branch_id`; `assigned_to_staff_id`; `converted_member_id`; `leads_created_by_staff_id_idx` (Phase 6); unique partial `(tenant_id, creation_request_key)` and `(tenant_id, conversion_request_key)` where the key is not null (Phase 6 — durable request evidence, `GL062` on a reused key with different facts or actor); `leads_tenant_id_updated_at_id_idx` on `(tenant_id, updated_at desc, id desc)` (Phase 6 — the list's keyset order).
- `created_by_staff_id` carries a composite foreign key, `leads_tenant_id_created_by_staff_id_fkey` on `(tenant_id, created_by_staff_id)` → `staff (tenant_id, id)` (ADR-052).
- RLS: Phase 6 replaces the Phase 1 `leads_tenant_all` with `leads_tenant_select` and `leads_tenant_write`, gated on real front office — an authenticated, **non-impersonating** `gym_owner`, `gym_manager` or `front_desk` with a same-tenant `staff_id`: both policies require `(select app.current_impersonation_id()) is null`, so a preview session reads no lead, and its writes are refused with `42501` `Support preview is read-only.` by the statement-level `leads_preview_write_guard` — a policy filters rather than raises, so a preview UPDATE would match no rows and fire no row-level trigger (ADR-118). Trainers and members hold no lead policy at all; platform support keeps its cross-gym read and cannot mutate.
- Phase 6 direct-row invariants, enforced on RPC and direct writes by `app.enforce_lead_discipline()` (the recreated `leads_touch_updated_at` trigger): a lead is created only at `new` with null trial/conversion/loss fields; `trial_at` is required exactly on `trial_scheduled`/`trial_done`; `lost` requires a trimmed nonempty reason and every other stage forbids one; `converted` requires a same-tenant eligible member (status not `cancelled`/`blocked`, `erased_at` null) and server conversion time, while every other stage requires both conversion fields null; a terminal row's business facts cannot change; `id`, `tenant_id`, `created_at` and both evidence pairs are immutable. These invariants bind **every** writer including the BYPASSRLS roles (ADR-120) — only the creator stamp is claim-keyed — and a fresh row's `created_at` and `updated_at` are server-stamped on INSERT; history rows enter only through an explicit bypass the writer states out loud (`session_replication_role = replica` for the fixtures, `ALTER TABLE … DISABLE TRIGGER` for the seed, ADR-098's pattern).
- `revision uuid` — database-owned and nonnull, replaced by a fresh UUID on every accepted material change and by nothing else; no-op writes do not rotate it and clients cannot set it. Every detail update, transition and conversion compares `expectedRevision` and rotates it atomically; a miss on a visible lead answers HTTP 409 `stale_lead` with the current revision, and an invisible or unknown lead is a generic 404. UUID revisions avoid counters and JSON number concerns.
- Creation and conversion evidence: a key/facts pair is either both null or both nonnull. New leads require complete creation evidence (original actor staff id, branch, normalized name/phone/email, source, assignee, notes); a new RPC conversion requires complete conversion evidence (original actor staff id, lead id, expected revision, mode, link member id), while historical converted rows may keep null evidence. A retry by another actor conflicts even if every other fact matches (`GL062`).
...
- **`membership_status`**: `pending`, `active`, `frozen`, `expired`, `cancelled`. Transitions: pending→active (verified payment only, PAY-008), pending→cancelled, active→frozen, frozen→active, active→expired, frozen→expired, active→cancelled, frozen→cancelled. `expired` and `cancelled` are terminal, **and since ADR-098 something enforces it** (`GL047`, `app.membership_transition_allowed()`) — this line described an intention rather than a rule from Phase 1 until then — a critic measured an ordinary front-desk session writing `cancelled → active`, and writing `expired` onto a membership live for another ten days. `memberships` now has the equivalent of `payments`' `app.payment_transition_allowed()` — closing **OPEN-030**, which the three rules depending on the answer had been resting on: the retired-membership rule (ADR-096), the check-in gate (ADR-084) and the one-live-membership index. A membership retired in error can no longer be typed back; the repair is a new membership. **And `expired` means three different things across three documents the routing table points at**: a reachable terminal state here, a state "nothing in this product ever writes" in ADR-064/075 — quoted verbatim inside the live `app.enforce_check_in()` body — and "retired, do not grant" in ADR-096. `supabase/seed-scenarios.sql` writes it, so the demo gym's showcase lapsed member holds it and her renewal is recorded, receipted and grants nothing; the console does say "This member has no membership to renew", so no money goes wrong, but nothing says that is the intended shape. A renewal is a new row.
...
- **`payment_status`**: `created`, `pending`, `paid`, `failed`, `refunded`, `reversed`. Transitions: created→pending, created→paid, pending→paid, created→failed, pending→failed, paid→refunded, paid→reversed. Never anything→paid except from created/pending on a verified provider event (PAY-007/008). `refunded` and `reversed` are terminal. **`failed` is NOT** — measured: `app.payment_transition_allowed('failed','created')` and `('failed','pending')` are both true, deliberately, so a failed attempt can be retried. This line called it terminal and listed neither edge.
...
- `refund_kind`: `refund`, `reversal`. `refund_status`: `requested`, `processing`, `completed`, `failed` (requested→processing→completed|failed; requested→failed). **`completed` is terminal and enforced** (`GL041`, round seventeen): the ceiling sums refunds that are not `failed`, so demoting a completed refund freed the room it was consuming and a second full refund was accepted — money that left becoming an attempt that never happened. `processing→failed` stays permitted, because a refund in flight can genuinely fail; what bounds that case is the ceiling running on UPDATE, so at most one of two refunds ever reaches `completed`. Refund identity, amount, currency and kind are permanent. A non-failed add-on return must use its linked payment currency; requested/processing rows reserve ceiling headroom, while only completed rows count as returned cash and drive fulfilment effects.
...
- `checkin_gate_mode`: `printed_poster`, `rotating_screen` (ATT-003/009–014). Gym owner/manager switches modes through an audited command; each QR session records its issuance mode.
...
- `lead_source`: `walk_in`, `referral`, `instagram`, `google`, `website`, `phone`, `other`. `lead_stage`: `new`, `contacted`, `trial_scheduled`, `trial_done`, `converted`, `lost` (new→contacted→trial_scheduled→trial_done→converted; any non-terminal→lost). Enforced since the Phase 6 leads cluster on RPC and direct writes (`GL059` illegal edge, `GL060` invalid facts); self-transitions are refused and `converted`/`lost` are terminal.
