...
| # | Gate | Automated? | Enforced by | Status |
|---|---|---|---|---|
| 12 | Zod validation and a typed error envelope on every endpoint | Yes (once endpoints exist) | Route Handler convention + review; no lint rule enforces this mechanically yet | N/A — **moved to Phase 3.** Phase 2 built the identity and authorization layer and no Route Handlers, so there is still nothing to validate. OPEN-002 (`packages/api-client`) moved with it, for the same reason and deliberately rather than by drift |
