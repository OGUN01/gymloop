...
| Member add-on returns | Keep direct member SELECT on `refunds` denied. One STABLE, postgres-owned, empty-search-path security-definer function accepts only an order id, requires a complete verified member identity, rechecks order/payment tenant and member independently, and returns only completed return id/kind/exact amount/currency/time. Missing and foreign facts are indistinguishable; no reason, actor, provider reference, request key or pending attempt is exposed. |
