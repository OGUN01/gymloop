...
| P6-D02 | What does PT “availability before payment” mean? | A PT sale must name an active trainer and an initial session slot; the overlap constraint must accept that slot before payment is recorded. A trainer may create or change only sessions assigned to their own verified staff id, checked against both OLD and NEW assignment. Every session must reference an order whose product kind is `pt_package` and equal that order's frozen member and trainer identities. The pending order and slot are one short transaction, so no unpaid hold survives a failure. | `ADD-003` requires a check, but the current contract does not define availability. |
...
| P6-D10 | Gym claim eligibility | Only an `active` gym, or a `trial` gym with a nonnull future trial end, receives fresh gym-side claims. Pending approval, suspended, closed and expired/malformed trials do not. Suspension/closure revokes refresh sessions; every existing access token has at most the configured fifteen-minute residual, with reserved `exp` unchanged. Reactivation is allowed only from `suspended`. | Today organization status is a label, not an identity gate. Making it an operational control changes the claim contract and requires the full blind process. |
...
- **ADD-009** WHEN a positive-price product order becomes paid THE SYSTEM SHALL decrement stock exactly once by its quantity and advance the order through `active` to `completed`; IF stock is insufficient THEN THE SYSTEM SHALL record neither a paid payment nor a completed order (ADD-004). A complimentary order SHALL retain its attributed keyed snapshot and normal stock, booking or usage effect without creating a zero-valued payment or receipt; its snapshot freezes and `sold_at` is server-stamped when it first leaves pending.
- **ADD-010** WHEN a PT order is submitted for payment THE SYSTEM SHALL require an active trainer and an initial session slot accepted by the no-overlap constraint before recording payment; IF the slot is unavailable THEN THE SYSTEM SHALL record neither paid payment nor live booking (ADD-003). Every PT session SHALL reference an order whose product kind is `pt_package` and equal that order's frozen member and trainer identities.
- **ADD-011** WHEN a PT session becomes `completed` THE SYSTEM SHALL increment its order's `sessions_used` exactly once; cancelled/no-show sessions SHALL consume none, and the final used session SHALL complete the order. A retry SHALL not consume a second session. A trainer may create or change only a session assigned to their own verified staff id; an update SHALL check both OLD and NEW assignment, while the existing staff-wide RLS policy retains administrative access for owner, manager and front office.
...
- **COM-001** WHEN a consent decision is recorded THE SYSTEM SHALL lock that member row, stamp `recorded_at` on the server from `clock_timestamp()`, and make it strictly later than the last decision for that member and purpose by one database timestamp tick when necessary. It SHALL append a versioned row and derive member/tenant/actor from verified context. Current consent SHALL be ordered by `recorded_at DESC, id DESC`; history SHALL remain immutable.
- **COM-002** THE SYSTEM SHALL classify renewal/payment/fulfilment messages as `service`, promotions as `marketing`, and motivation messages as service messages additionally gated by `motivation_push_enabled`. IF the applicable consent is absent or withdrawn THEN THE SYSTEM SHALL create no send attempt, or SHALL move an already-scheduled one to `opted_out` before sending. Consent insertion and the send decision SHALL take the same member-row lock; the sender SHALL read current consent in a fresh command after acquiring it, so a serialized withdrawal wins.
...
The owner snapshot SHALL be one read-only `security invoker` SQL statement. It
returns cards and exact component rows from the same snapshot; nested read
helpers are STABLE. Every bigint/numeric integer component and aggregate crosses
the JSON boundary as a decimal string and remains exact through display.
...
- **MET-007** “Add-on net revenue” SHALL use payments linked from add-on orders minus their completed refunds/reversals in the selected range. “PT utilisation” SHALL show `sum(sessions_used) / sum(sessions_total)` over the selected range's paid/active/completed, non-refunded PT-order cohort, with both values visible. Positive-price PT cohorts use the linked payment's `paid_at`; complimentary PT cohorts use immutable `sold_at`, contribute usage with zero revenue, and undated historical orders remain explicitly undated rather than being assigned to an arbitrary cohort.
...
- **OPS-003** WHEN super admin starts impersonation THE SYSTEM SHALL require the existing reason and hard expiry, attribute the session to the actor, and show the persistent red banner for its duration. The database SHALL refuse direct authenticated table mutations carrying an impersonation claim while preserving reads and the existing own-session end exception. Support SHALL have no start control.
...
- **ONB-001** WHEN super admin creates a gym THE SYSTEM SHALL create the organization, exactly one `organization_settings` row, one existing same-gym default branch, one messaging wallet, and one owner staff row in one transaction. Failure of any child write SHALL create none, closing OPEN-018. Activation uses that default-branch record and SHALL not require an invented active-branch flag.
- **ONB-002** THE SYSTEM SHALL generate a unique six-character gym code, set trial end at the start of the gym-local date fourteen days after onboarding, and keep activation manual. Activation SHALL be refused until settings, exactly one same-gym default branch, an active owner linked to an existing non-platform Auth user, timezone and currency are present.
- **ONB-003** WHEN a preset is selected THE SYSTEM SHALL copy these adopted defaults once:
  ...
  |---|---:|---|---:|---:|---|
  | `neighbourhood_gym` | 7 days | `visit_streak` | 3 | 30 days | `gym_owner` |
  | `premium_studio` | 5 days | `weekly_goal` | 3 | 30 days | `gym_owner` |
  | `functional_box` | 3 days | `weekly_goal` | 4 | 14 days | `gym_owner` |

  ADR-113 sets the initial approver to the owner because onboarding creates no manager. The owner may configure a manager later. GL021 still requires different requester and approver, so onboarding one owner does not promise immediate solo pause approval.

- **ONB-004** WHEN onboarding has no linked owner auth user or no invitation provider THE SYSTEM SHALL show “account access pending”, SHALL send nothing, and SHALL keep activation unavailable. Linking SHALL require one existing Auth user selected by normalized exact email, reject ambiguous matches and any platform identity, atomically set that user's protected `active_tenant_id` preferred tenant to the target gym, revoke old and new linked refresh sessions on replacement (or the new user's on first link), and be explicit and audited. An unchanged exact link SHALL be inert.
...
2. Land the reviewed identity/navigation slice with complete claim classification, claim cleanup, database read-only previews, and working member/platform read homes. NAV-006/008 organization eligibility and protected commercial controls land together in the later platform slice. Its exact money-display extension uses the full independent arrangement with the identity work.
3. With the shared contracts fixed, build vertical clusters B (leads and the shared local-time input), C (imports), and the provider-independent part of D (consent/in-app/WhatsApp truthfulness, closing OPEN-010). Independent work may run in parallel; each coherent cluster lands after its predecessor's gates and owns its route, screen and API schemas end to end.
